...
export function transformClassFields(context: TransformationContext) {
    ...
    function visitPropertyDeclaration(node: PropertyDeclaration) {
        ...
        if (!shouldTransformPrivateFields && isPrivateIdentifier(node.name)) {
            ...
            /*decorators*/ undefined, visitNodes(node.modifiers, visitor, isModifier), node.name, 
    ...
    function visitPostfixUnaryExpression(node: PostfixUnaryExpression, valueIsDiscarded: boolean) {
        if (shouldTransformPrivateFields && isPrivateIdentifierPropertyAccessExpression(node.operand)) {
            ...
            if (operator && (info = accessPrivateIdentifier(node.operand.name))) {
                ...
                return setOriginalNode(inlineExpressions(compact<Expression>([
                    createPrivateIdentifierAssignment(info, initializeExpression || readExpression, createBinary(returnValue ? createAssignment(returnValue, existingValue) : existingValue, operator, createLiteral(1)), SyntaxKind.EqualsToken),
                    returnValue
                ])), node);
            }
        }
    ...
    function visitCallExpression(node: CallExpression) {
        if (shouldTransformPrivateFields && isPrivateIdentifierPropertyAccessExpression(node.expression)) {
            ...
            return updateCall(node, createPropertyAccess(visitNode(target, visitor), "call"), 
    ...
    function visitTaggedTemplateExpression(node: TaggedTemplateExpression) {
        if (shouldTransformPrivateFields && isPrivateIdentifierPropertyAccessExpression(node.tag)) {
            ...
            return updateTaggedTemplate(node, createCall(createPropertyAccess(visitNode(target, visitor), "bind"), 
            /*typeArguments*/ undefined, [visitNode(thisArg, visitor, isExpression)]), visitNode(node.template, visitor, isTemplateLiteral));
        }
        return visitEachChild(node, visitor, context);
    }
    ...
    function visitClassDeclaration(node: ClassDeclaration) {
        ...
        const statements: Statement[] = [
            updateClassDeclaration(node, 
            /*decorators*/ undefined, node.modifiers, node.name, 
            /*typeParameters*/ undefined, visitNodes(node.heritageClauses, visitor, isHeritageClause), transformClassMembers(node, isDerivedClass))
        ];
    ...
    function visitClassExpression(node: ClassExpression): Expression {
        ...
        const extendsClauseElement = getEffectiveBaseTypeNode(node);
        const isDerivedClass = !!(extendsClauseElement && skipOuterExpressions(extendsClauseElement.expression).kind !== SyntaxKind.NullKeyword);
        const classExpression = updateClassExpression(node, node.modifiers, node.name, 
        /*typeParameters*/ undefined, visitNodes(node.heritageClauses, visitor, isHeritageClause), transformClassMembers(node, isDerivedClass));
    ...
    function transformConstructor(node: ClassDeclaration | ClassExpression, isDerivedClass: boolean) {
        ...
        return startOnNewLine(setOriginalNode(setTextRange(createConstructor(
    ...
    function transformConstructorBody(node: ClassDeclaration | ClassExpression, constructor: ConstructorDeclaration | undefined, isDerivedClass: boolean) {
        ...
        if (!constructor && isDerivedClass) {
            ...
            //  super(...arguments);
            //
            statements.push(createExpressionStatement(createCall(createSuper(), 
            /*typeArguments*/ undefined, [createSpread(createIdentifier("arguments"))])));
        }
        if (constructor) {
        ...
        return setTextRange(createBlock(setTextRange(createNodeArray(statements), 
    ...
    function addPrivateIdentifierToEnvironment(name: PrivateIdentifier) {
        ...
        (pendingExpressions || (pendingExpressions = [])).push(createAssignment(weakMapName, createNew(createIdentifier("WeakMap"), 
    ...
    function wrapPrivateIdentifierForDestructuringTarget(node: PrivateIdentifierPropertyAccessExpression) {
        ...
        createParen(createObjectLiteral([
            createSetAccessor(
            /*decorators*/ undefined, 
            /*modifiers*/ undefined, "value", [createParameter(
                /*decorators*/ undefined, 
                /*modifiers*/ undefined, 
                /*dotDotDotToken*/ undefined, parameter, 
                /*questionToken*/ undefined, 
                /*type*/ undefined, 
                /*initializer*/ undefined)], createBlock([createExpressionStatement(createPrivateIdentifierAssignment(info, receiver, parameter, SyntaxKind.EqualsToken))]))
        ])), "value");
    }
...
function createPrivateInstanceFieldInitializer(receiver: LeftHandSideExpression, initializer: Expression | undefined, weakMapName: Identifier) {
    return createCall(createPropertyAccess(weakMapName, "set"), 
    /*typeArguments*/ undefined, [receiver, initializer || createVoidZero()]);
}
/* @internal */
