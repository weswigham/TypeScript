...
import {getDeclarationDiagnostics as getEmitterDeclarationDiagnostics} from "./declarationEmitter";
import {
    createSourceFile, forEachChild
} from "./parser";
import {
    ModuleKind, ModuleResolutionKind, CompilerOptions,
    ModuleResolutionHost, ResolvedModule, CharacterCodes,
    ScriptTarget, Map, CompilerHost,
    SourceFile, Program, CancellationToken,
    Diagnostic, DiagnosticMessageChain, TypeChecker,
    WriteFileCallback, EmitResult, OperationCanceledException,
    FileReference, LiteralExpression, SyntaxKind,
    ModuleDeclaration, NodeFlags, DiagnosticMessage,
    Expression
} from "./types";
import {
    Debug, createCompilerDiagnostic, hasProperty,
    getDirectoryPath, isRootedDiskPath, combinePaths,
    normalizePath, getRootLength, forEach,
    supportedExtensions, fileExtensionIs, normalizeSlashes,
    getBaseFileName, sortAndDeduplicateDiagnostics, map,
    createFileMap, copyMap, addRange,
    createFileDiagnostic, getNormalizedAbsolutePath, getNormalizedPathComponents,
    getNormalizedPathFromPathComponents, directorySeparator
} from "./core";
import {
    getNewLineCharacter, getDefaultLibFileName, createDiagnosticCollection,
    arrayIsEqualTo, getResolvedModuleFileName, EmitHost,
    isDeclarationFile, getExternalModuleName, isExternalModuleImportEqualsDeclaration,
    getExternalModuleImportEqualsDeclarationExpression, setResolvedModuleName, isExternalModule,
    getErrorSpanForNode
} from "./utilities";
import {Diagnostics} from "./diagnosticInformationMap.generated";

/* @internal */ export const timings = {
    programTime: 0,
    emitTime: 0,
    ioReadTime: 0,
    ioWriteTime: 0
...
export function createCompilerHost(options: CompilerOptions, setParentNodes?: boolean): CompilerHost {
    ...
    function getSourceFile(fileName: string, languageVersion: ScriptTarget, onError?: (message: string) => void): SourceFile {
        ...
        try {
            ...
            timings.ioReadTime += new Date().getTime() - start;
        }
        catch (e) {
            if (onError) {
    ...
    function writeFile(fileName: string, data: string, writeByteOrderMark: boolean, onError?: (message: string) => void) {
        try {
            ...
            timings.ioWriteTime += new Date().getTime() - start;
        }
        catch (e) {
            if (onError) {
                onError(e.message);
...
export function createProgram(rootNames: string[], options: CompilerOptions, host?: CompilerHost, oldProgram?: Program): Program {
    ...
    timings.programTime += new Date().getTime() - start;
    ...
    function emitWorker(program: Program, sourceFile: SourceFile, writeFileCallback: WriteFileCallback, cancellationToken: CancellationToken): EmitResult {
        ...
        timings.emitTime += new Date().getTime() - start;
        return emitResult;
    }
    ...
    function getDeclarationDiagnosticsForFile(sourceFile: SourceFile, cancellationToken: CancellationToken): Diagnostic[] {
        return runWithCancellationToken(() => {
            if (!isDeclarationFile(sourceFile)) {
                ...
                return getEmitterDeclarationDiagnostics(getEmitHost(writeFile), resolver, sourceFile);
            }
        });
    }
