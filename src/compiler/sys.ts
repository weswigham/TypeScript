...
export function createDirectoryWatcherSupportingRecursive(host: RecursiveDirectoryWatcherHost): HostWatchDirectory {
    ...
    const cacheToUpdateChildWatches = createMap<{
        dirName: string;
    ...
    function invokeCallbacks(dirPath: Path, fileNameOrInvokeMap: string | ts.Map<true>) {
    ...
    function watchChildDirectories(parentDir: string, existingChildWatches: ChildWatches, options: WatchOptions | undefined): ChildWatches {
        ...
        enumerateInsertsAndDeletes<string, ChildDirectoryWatcher>(host.directoryExists(parentDir) ? mapDefined(host.getAccessibleSortedChildDirectories(parentDir), child => {
        ...
        }) : emptyArray, existingChildWatches, (child, childWatcher) => filePathComparer(child, childWatcher.dirName), createAndAddChildDirectoryWatcher, closeFileWatcher, addChildDirectoryWatcher);
