import {
    SourceFile as OriginalSourceFile, Diagnostic, Program,
    DiagnosticMessage, DiagnosticCategory, CompilerHost,
    CompilerOptions, ExitStatus, ScriptTarget,
    CommandLineOption, Map
} from "./types";
import {
    getLineStarts, getLineAndCharacterOfPosition
} from "./scanner";
import {
    flattenDiagnosticMessageText, findConfigFile, version,
    createCompilerHost, createProgram, defaultInitCompilerOptions,
    timings
} from "./program";
import {timings as bindTimings} from "./binder";
import {timings as checkerTimings} from "./checker";
import {
    parseCommandLine, parseConfigFile, readConfigFile,
    optionDeclarations, getOptionNameMap
} from "./commandLineParser";
import {
    createCompilerDiagnostic, normalizePath, getDirectoryPath,
    combinePaths, forEach, extend,
    contains, filter, compareValues,
    hasProperty, localizedDiagnosticMessages, forEachKey
} from "./core";
...
function trySetLanguageAndTerritory(language: string, territory: string, errors: Diagnostic[]): boolean {
    ...
    try {
        let msgs = JSON.parse(fileContents);
        forEachKey(localizedDiagnosticMessages, key => {
            localizedDiagnosticMessages[key] = undefined;
        });
        forEachKey(msgs, key => {
            localizedDiagnosticMessages[key] = msgs[key];
        });
    }
    catch (e) {
...
export function executeCommandLine(args: string[]): void {
    ...
    function getSourceFile(fileName: string, languageVersion: ScriptTarget, onError?: (message: string) => void) {
        ...
        if (cachedProgram) {
            let sourceFile: SourceFile = cachedProgram.getSourceFile(fileName);
        ...
        let sourceFile: SourceFile = hostGetSourceFile(fileName, languageVersion, onError);
    ...
    function setCachedProgram(program: Program) {
        if (cachedProgram) {
            ...
            forEach(cachedProgram.getSourceFiles(), (sourceFile: SourceFile) => {
...
function compile(fileNames: string[], compilerOptions: CompilerOptions, compilerHost: CompilerHost) {
    timings.ioReadTime = 0;
    timings.ioWriteTime = 0;
    timings.programTime = 0;
    bindTimings.bindTime = 0;
    checkerTimings.checkTime = 0;
    timings.emitTime = 0;
    ...
    if (compilerOptions.diagnostics) {
        ...
        reportTimeStatistic("I/O read", timings.ioReadTime);
        reportTimeStatistic("I/O write", timings.ioWriteTime);
        reportTimeStatistic("Parse time", timings.programTime);
        reportTimeStatistic("Bind time", bindTimings.bindTime);
        reportTimeStatistic("Check time", checkerTimings.checkTime);
        reportTimeStatistic("Emit time", timings.emitTime);
        reportTimeStatistic("Total time", timings.programTime + bindTimings.bindTime + checkerTimings.checkTime + timings.emitTime);
    }
