namespace ts.server.typingsInstaller {
    ...
    function loadTypesRegistryFile(typesRegistryFilePath: string, host: ts.server.InstallTypingHost, log: ts.server.typingsInstaller.Log): ts.Map<ts.MapLike<string>> {
        if (!host.fileExists(typesRegistryFilePath)) {
            if (log.isEnabled()) {
                log.writeLine(`Types registry file '${typesRegistryFilePath}' does not exist`);
            }
            return ts.createMap<ts.MapLike<string>>();
        }
        try {
    ...
    export class NodeTypingsInstaller extends ts.server.typingsInstaller.TypingsInstaller {
        ...
        readonly typesRegistry: ts.Map<ts.MapLike<string>>;
