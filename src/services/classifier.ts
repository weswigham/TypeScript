namespace ts {
    ...
    function convertClassificationsToResult(classifications: ts.Classifications, text: string): ts.ClassificationResult {
        ...
        for (let i = 0; i < dense.length; i += 3) {
            ...
            const type = (<ts.ClassificationType>dense[i + 2]);
    ...
    function getPrefixFromLexState(lexState: ts.EndOfLineState): {
    ...
    /* @internal */
    export function getSemanticClassifications(typeChecker: ts.TypeChecker, cancellationToken: ts.CancellationToken, sourceFile: ts.SourceFile, classifiableNames: ts.UnderscoreEscapedMap<true>, span: ts.TextSpan): ts.ClassifiedSpan[] {
    ...
    /* @internal */
    export function getEncodedSemanticClassifications(typeChecker: ts.TypeChecker, cancellationToken: ts.CancellationToken, sourceFile: ts.SourceFile, classifiableNames: ts.UnderscoreEscapedMap<true>, span: ts.TextSpan): ts.Classifications {
        const spans: number[] = [];
    ...
    function hasValueSideModule(symbol: ts.Symbol): boolean {
        return ts.some(symbol.declarations, declaration => ts.isModuleDeclaration(declaration) && ts.getModuleInstanceState(declaration) === ts.ModuleInstanceState.Instantiated);
    }
    ...
    export function getEncodedSyntacticClassifications(cancellationToken: ts.CancellationToken, sourceFile: ts.SourceFile, span: ts.TextSpan): ts.Classifications {
        ...
        function classifyComment(token: ts.Node, kind: ts.SyntaxKind, start: number, width: number) {
            if (kind === ts.SyntaxKind.MultiLineCommentTrivia) {
                ...
                if (docCommentAndDiagnostics && docCommentAndDiagnostics.jsDoc) {
                    ...
                    docCommentAndDiagnostics.jsDoc.parent = (token as ts.HasJSDoc);
                ...
                for (const tag of docComment.tags) {
                    ...
                    switch (tag.kind) {
                        case ts.SyntaxKind.JSDocParameterTag:
                            processJSDocParameterTag((<ts.JSDocParameterTag>tag));
                            break;
                        case ts.SyntaxKind.JSDocTemplateTag:
                            processJSDocTemplateTag((<ts.JSDocTemplateTag>tag));
                            pos = tag.end;
                            break;
                        case ts.SyntaxKind.JSDocTypeTag:
                            processElement((<ts.JSDocTypeTag>tag).typeExpression);
                            pos = tag.end;
                            break;
                        case ts.SyntaxKind.JSDocReturnTag:
                            processElement((<ts.JSDocReturnTag>tag).typeExpression);
                            pos = tag.end;
                            break;
                    }
                }
        ...
        function tryClassifyTripleSlashComment(start: number, width: number): boolean {
            ...
            while (true) {
                ...
                pushClassification(attrPos, attrMatch[1].length, ts.ClassificationType.jsxAttribute); // attribute name
                attrPos += attrMatch[1].length;
                if (attrMatch[2].length) {
                    pushCommentRange(attrPos, attrMatch[2].length); // whitespace
                    attrPos += attrMatch[2].length;
                }
                pushClassification(attrPos, attrMatch[3].length, ts.ClassificationType.operator); // =
                attrPos += attrMatch[3].length;
                if (attrMatch[4].length) {
                    pushCommentRange(attrPos, attrMatch[4].length); // whitespace
                    attrPos += attrMatch[4].length;
                }
                pushClassification(attrPos, attrMatch[5].length, ts.ClassificationType.jsxAttributeStringLiteralValue); // attribute value
        ...
        function tryClassifyJsxElementName(token: ts.Node): ts.ClassificationType | undefined {
            switch (token.parent && token.parent.kind) {
                case ts.SyntaxKind.JsxOpeningElement:
                    if ((<ts.JsxOpeningElement>token.parent).tagName === token) {
                        return ts.ClassificationType.jsxOpenTagName;
                    }
                    break;
                case ts.SyntaxKind.JsxClosingElement:
                    if ((<ts.JsxClosingElement>token.parent).tagName === token) {
                        return ts.ClassificationType.jsxCloseTagName;
                    }
                    break;
                case ts.SyntaxKind.JsxSelfClosingElement:
                    if ((<ts.JsxSelfClosingElement>token.parent).tagName === token) {
                        return ts.ClassificationType.jsxSelfClosingTagName;
                    }
                    break;
                case ts.SyntaxKind.JsxAttribute:
                    if ((<ts.JsxAttribute>token.parent).name === token) {
            ...
            if (ts.isPunctuation(tokenKind)) {
                ...
                if (token) {
                    switch (token.parent.kind) {
                        case ts.SyntaxKind.ClassDeclaration:
                            if ((<ts.ClassDeclaration>token.parent).name === token) {
                                return ts.ClassificationType.className;
                            }
                            return;
                        case ts.SyntaxKind.TypeParameter:
                            if ((<ts.TypeParameterDeclaration>token.parent).name === token) {
                                return ts.ClassificationType.typeParameterName;
                            }
                            return;
                        case ts.SyntaxKind.InterfaceDeclaration:
                            if ((<ts.InterfaceDeclaration>token.parent).name === token) {
                                return ts.ClassificationType.interfaceName;
                            }
                            return;
                        case ts.SyntaxKind.EnumDeclaration:
                            if ((<ts.EnumDeclaration>token.parent).name === token) {
                                return ts.ClassificationType.enumName;
                            }
                            return;
                        case ts.SyntaxKind.ModuleDeclaration:
                            if ((<ts.ModuleDeclaration>token.parent).name === token) {
                                return ts.ClassificationType.moduleName;
                            }
                            return;
                        case ts.SyntaxKind.Parameter:
                            if ((<ts.ParameterDeclaration>token.parent).name === token) {
