...
import {
    ...
    Signature as OriginalSignature,
    SourceFile as OriginalSourceFile,
    SymbolFlags, Declaration, TypeFlags,
    ObjectType, SignatureDeclaration, Map,
    LineAndCharacter, TextChangeRange, FileReference,
    ScriptTarget, SyntaxKind, NodeFlags,
    NodeArray, TextRange, ModuleDeclaration
} from "../compiler/types";
import {
    Scanner, createScanner
} from "../compiler/scanner";
import {getSourceFileOfNode, getTokenPosOfNode} from "../compiler/utilities";
import {getNodeConstructor, forEachChild} from "../compiler/parser";
import {
    lastOrUndefined, forEach, indexOf,
    addRange
} from "../compiler/core";
...
export interface Symbol extends OriginalSymbol {
...
export interface Type extends OriginalType {
    getFlags(): TypeFlags;
    getSymbol(): Symbol;
...
export interface Signature extends OriginalSignature {
...
export interface SourceFile extends OriginalSourceFile {
...
class NodeObject implements Node {
    ...
    public getSourceFile(): SourceFile {
        return <SourceFile>getSourceFileOfNode(this);
    }
...
function getJsDocCommentsFromDeclarations(declarations: Declaration[], name: string, canUseParsedParamTagComments: boolean) {
    ...
    forEach(docComments, docComment => {
    ...
    function getJsDocCommentsSeparatedByNewLines() {
        ...
        forEach(declarations, (declaration, indexOfDeclaration) => {
            ...
            if (indexOf(declarations, declaration) === indexOfDeclaration) {
                ...
                if (canUseParsedParamTagComments && declaration.kind === SyntaxKind.Parameter) {
                    forEach(getJsDocCommentTextRange(declaration.parent, sourceFileOfDeclaration), jsDocCommentTextRange => {
                ...
                forEach(getJsDocCommentTextRange(
        ...
        function getJsDocCommentTextRange(node: Node, sourceFile: SourceFile): TextRange[] {
            return map(getJsDocComments(node, sourceFile),
                jsDocComment => {
                    return {
...
export function createLanguageService(host: LanguageServiceHost, documentRegistry: DocumentRegistry = createDocumentRegistry()): LanguageService {
    ...
    function getCompletionsAtPosition(fileName: string, position: number): CompletionInfo {
        ...
        function getAllJsDocCompletionEntries(): CompletionEntry[] {
            return jsDocCompletionEntries || (jsDocCompletionEntries = map(jsDocTagNames, tagName => {
                return {
                    name: tagName,
                    kind: ScriptElementKind.keyword,
                    kindModifiers: "",
    ...
    function getOccurrencesAtPosition(fileName: string, position: number): ReferenceEntry[] {
        ...
        if (results) {
            ...
            results = filter(results, r => getCanonicalFileName(normalizeSlashes(r.fileName)) === sourceFile);
        }
    ...
    function getNavigateToItems(searchValue: string, maxResultCount?: number): NavigateToItem[] {
        ...
        return NavigateTo.getNavigateToItems(program, cancellationToken, searchValue, maxResultCount);
    }
    ...
    function getBraceMatchingAtPosition(fileName: string, position: number) {
        ...
        function getMatchingTokenKind(token: Node): SyntaxKind {
            switch (token.kind) {
                case SyntaxKind.OpenBraceToken: return SyntaxKind.CloseBraceToken
                case SyntaxKind.OpenParenToken: return SyntaxKind.CloseParenToken;
                case SyntaxKind.OpenBracketToken: return SyntaxKind.CloseBracketToken;
                case SyntaxKind.LessThanToken: return SyntaxKind.GreaterThanToken;
                case SyntaxKind.CloseBraceToken: return SyntaxKind.OpenBraceToken
                case SyntaxKind.CloseParenToken: return SyntaxKind.OpenParenToken;
                case SyntaxKind.CloseBracketToken: return SyntaxKind.OpenBracketToken;
                ...
                if (declarations && declarations.length > 0) {
                    ...
                    if (defaultLibFileName) {
                        for (let current of declarations) {
                            ...
                            var canonicalName = getCanonicalFileName(normalizePath(sourceFile.fileName));
                            if (sourceFile && getCanonicalFileName(normalizePath(sourceFile.fileName)) === getCanonicalFileName(normalizePath(defaultLibFileName))) {
