import {
    CallLikeExpression, TextSpan, Program,
    CancellationToken, SyntaxKind, Identifier,
    CallExpression, PropertyAccessExpression, TemplateExpression,
    LiteralExpression, TaggedTemplateExpression, TemplateSpan
} from "../compiler/types";
import {
    SourceFile, Signature, Node,
    SignatureHelpItems, Type
} from "./services";
import {
    Debug, getProperty, countWhere,
    lastOrUndefined
} from "../compiler/core";
import {isJavaScript, isTemplateLiteralKind} from "../compiler/utilities";
import {
    findTokenOnLeftOfPosition, findListItemInfo, isInsideTemplateLiteral,
...
export function getSignatureHelpItems(program: Program, sourceFile: SourceFile, position: number, cancellationToken: CancellationToken): SignatureHelpItems {
    ...
    let resolvedSignature = <Signature>typeChecker.getResolvedSignature(call, candidates);
    cancellationToken.throwIfCancellationRequested();
    ...
    function createJavaScriptSignatureHelpItems(argumentInfo: ArgumentListInfo): SignatureHelpItems {
        ...
        let typeChecker = program.getTypeChecker();
        for (let sourceFile of <SourceFile[]>program.getSourceFiles()) {
            ...
            if (declarations) {
                for (let declaration of declarations) {
                    ...
                    if (symbol) {
                        let type = <Type>typeChecker.getTypeOfSymbolAtLocation(symbol, declaration);
                        if (type) {
    ...
    function getImmediatelyContainingArgumentInfo(node: Node): ArgumentListInfo {
        if (node.parent.kind === SyntaxKind.CallExpression || node.parent.kind === SyntaxKind.NewExpression) {
            ...
            if (node.kind === SyntaxKind.LessThanToken ||
                ...
                let list = getChildListThatStartsWithOpenerToken(<any>callExpression, node, sourceFile);
            ...
            if (isInsideTemplateLiteral(<any>node, position)) {
            ...
            let argumentIndex = isInsideTemplateLiteral(<any>node, position) ? 0 : 1;
            ...
            if (node.kind === SyntaxKind.TemplateTail && !isInsideTemplateLiteral(<any>node, position)) {
                return undefined;
            }
