...
/* @internal */
export function isJumpStatementTarget(node: Node): node is Identifier & {
...
/* @internal */
export function insertImport(changes: ChangeTracker, sourceFile: SourceFile, importDecl: Statement, blankLineBetween: boolean): void {
...
/* @internal */
export function getSynthesizedDeepCloneWithRenames<T extends Node>(node: T, includeTrivia = true, renameMap?: ts.Map<Identifier>, checker?: TypeChecker, callback?: (originalNode: Node, clone: Node) => any): T {
    let clone;
    if (renameMap && checker && isBindingElement(node) && isIdentifier(node.name) && isObjectBindingPattern(node.parent)) {
        ...
        if (renameInfo && renameInfo.text !== (node.name || node.propertyName).getText()) {
            clone = createBindingElement(node.dotDotDotToken, node.propertyName || node.name, renameInfo, node.initializer);
        }
    }
...
/* @internal */
function getSynthesizedDeepCloneWorker<T extends Node>(node: T, renameMap?: ts.Map<Identifier>, checker?: TypeChecker, callback?: (originalNode: Node, clone: Node) => any): T {
...
/* @internal */
export const syntaxMayBeASICandidate = or(syntaxRequiresTrailingCommaOrSemicolonOrASI, syntaxRequiresTrailingFunctionBlockOrSemicolonOrASI, syntaxRequiresTrailingModuleBlockOrSemicolonOrASI, syntaxRequiresTrailingSemicolonOrASI);
/* @internal */
...
export function getNameForExportedSymbol(symbol: Symbol, scriptTarget: ScriptTarget) {
    if (symbol.escapedName === InternalSymbolName.ExportEquals || symbol.escapedName === InternalSymbolName.Default) {
        ...
        return firstDefined(symbol.declarations, d => isExportAssignment(d) && isIdentifier(d.expression) ? d.expression.text : undefined)
            || moduleSymbolToValidIdentifier(Debug.checkDefined(symbol.parent), scriptTarget);
    }
    return symbol.name;
}
