...
namespace ts.codefix {
    ...
    ts.codefix.registerCodeFix({
        ...
        getCodeActions(context) {
            ...
            const changes = ts.textChanges.ChangeTracker.with(context, t => addMissingMembers(getClass(sourceFile, span.start), sourceFile, context, t, context.preferences));
    ...
    function addMissingMembers(classDeclaration: ts.ClassLikeDeclaration, sourceFile: ts.SourceFile, context: ts.codefix.TypeConstructionContext, changeTracker: ts.textChanges.ChangeTracker, preferences: ts.UserPreferences): void {
        ...
        ts.codefix.createMissingMemberNodes(classDeclaration, abstractAndNonPrivateExtendsSymbols, context, preferences, member => changeTracker.insertNodeAtClassStart(sourceFile, classDeclaration, member));
    }
