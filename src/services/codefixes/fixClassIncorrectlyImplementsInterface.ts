...
namespace ts.codefix {
    ...
    ts.codefix.registerCodeFix({
        ...
        getCodeActions(context) {
            ...
            return ts.mapDefined<ts.ExpressionWithTypeArguments, ts.CodeFixAction>(ts.getClassImplementsHeritageClauseElements(classDeclaration), implementedTypeNode => {
        ...
        getAllCodeActions(context) {
            const seenClassDeclarations = ts.createMap<true>();
    ...
    function symbolPointsToNonPrivateMember(symbol: ts.Symbol) {
        return !(ts.getModifierFlags(symbol.valueDeclaration) & ts.ModifierFlags.Private);
    }
    function addMissingDeclarations(context: ts.codefix.TypeConstructionContext, implementedTypeNode: ts.ExpressionWithTypeArguments, sourceFile: ts.SourceFile, classDeclaration: ts.ClassLikeDeclaration, changeTracker: ts.textChanges.ChangeTracker, preferences: ts.UserPreferences): void {
        ...
        ts.codefix.createMissingMemberNodes(classDeclaration, nonPrivateAndNotExistedInHeritageClauseMembers, context, preferences, member => insertInterfaceMemberNode(sourceFile, classDeclaration, member));
        function createMissingIndexSignatureDeclaration(type: ts.InterfaceType, kind: ts.IndexKind): void {
            const indexInfoOfKind = checker.getIndexInfoOfType(type, kind);
            if (indexInfoOfKind) {
                insertInterfaceMemberNode(sourceFile, classDeclaration, (checker.indexInfoToIndexSignatureDeclaration(indexInfoOfKind, kind, classDeclaration, /*flags*/ undefined, ts.codefix.getNoopSymbolTrackerWithResolver(context))!));
            }
        }
