...
namespace ts.codefix {
    ...
    ts.codefix.registerCodeFix({
        ...
        getAllCodeActions(context) {
            ...
            const seenClasses = ts.createMap<true>(); // Ensure we only do this once per class.
    ...
    function getNodes(sourceFile: ts.SourceFile, pos: number): {
        ...
        readonly superCall: ts.ExpressionStatement;
    } | undefined {
    ...
    function findSuperCall(n: ts.Node): (ts.ExpressionStatement & {
        expression: ts.CallExpression;
    }) | undefined {
        return ts.isExpressionStatement(n) && ts.isSuperCall(n.expression)
            ? n as ts.ExpressionStatement & {
                expression: ts.CallExpression;
            }
            : ts.isFunctionLike(n)
                ? undefined
