...
declare namespace TypeFacts {
   ...
   */
  export type EQString<T> = T & string;

   /**
   * Include only number from T
   */
  export type EQNumber<T> = T & number;

   /**
   * Include only number from T
   */
  export type EQBigInt<T> = T & bigint;

   /**
   * Include only boolean from T
   */
  export type EQBoolean<T> = T & boolean;

   /**
   * Include only symbol from T
   */
  export type EQSymbol<T> = T & symbol;

   /**
   * Include only object from T
   */
  export type EQObject<T> = T & object;

   /**
   * Include only functions from T
   */
  export type EQFunction<T> = T & Function;

   /**
   * Exclude only string from T
   */
  export type NEString<T> = T & not string;

   /**
   * Exclude only number from T
   */
  export type NENumber<T> = T & not number;

   /**
   * Exclude only number from T
   */
  export type NEBigInt<T> = T & not bigint;

   /**
   * Exclude only boolean from T
   */
  export type NEBoolean<T> = T & not boolean;

   /**
   * Exclude only symbol from T
   */
  export type NESymbol<T> = T & not symbol;

   /**
   * Exclude only object from T
   */
  export type NEObject<T> = T & not object;

   /**
   * Exclude only functions from T
   */
  export type NEFunction<T> = T & not Function;

   /**
   * Include only undefined from T
   */
  export type EQUndefined<T> = T & (undefined | void);

   /**
   * Include only null from T
   */
  export type EQNull<T> = T & null;

   /**
   * Include only null and undefined from T
   */
  export type EQUndefinedOrNull<T> = T & (undefined | null | void);

   /**
   * Exclude only undefined from T
   */
  export type NEUndefined<T> = T & not (undefined | void);

   /**
   * Exclude only null from T
   */
  export type NENull<T> = T & not null;

   /**
   * Exclude null and undefined from T (different from NonNullable as it also filters void)
   */
  export type NEUndefinedOrNull<T> = T & not (undefined | null | void);

  type FalseValues = false | null | undefined | void | 0 | 0n | "";

   /**
   * Include truthy (or exclude falsy) types from T
   */
  export type Truthy<T> = T & not FalseValues;

   /**
   * Include falsy from T
   */
  export type Falsy<T> = T & FalseValues;
}
