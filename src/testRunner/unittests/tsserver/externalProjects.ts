namespace ts.projectSystem {
    describe("unittests:: tsserver:: ExternalProjects", () => {
        ...
        it("load global plugins", () => {
            ...
            session.executeCommand((<ts.projectSystem.protocol.OpenExternalProjectsRequest>{
                seq: 1,
                type: "request",
            ...
            const handlerResponse = session.executeCommand((<ts.projectSystem.protocol.SemanticDiagnosticsSyncRequest>{
                seq: 2,
                type: "request",
        ...
        describe("deleting config file opened from the external project works", () => {
            function verifyDeletingConfigFile(lazyConfiguredProjectsFromExternalProject: boolean) {
                ...
                ts.projectSystem.checkProjectActualFiles(configProject, lazyConfiguredProjectsFromExternalProject ? ts.emptyArray : // Since no files opened from this project, its not loaded
                    [configFile.path]);
                host.reloadFS([ts.projectSystem.libFile, site]);
