namespace ts.projectSystem {
    ...
    interface InstallerParams {
        ...
        typesRegistry?: ts.Map<ts.MapLike<string>>;
    }
    ...
    describe("unittests:: tsserver:: typingsInstaller:: General functionality", () => {
        it("configured projects (typings installed) 1", () => {
            ...
            ts.projectSystem.checkProjectActualFiles(p, [file1.path, tsconfig.path]);
            const expectedWatchedFiles = ts.createMap<number>();
            expectedWatchedFiles.set(tsconfig.path, 1); // tsserver
            expectedWatchedFiles.set(ts.projectSystem.libFile.path, 1); // tsserver
            ...
            const expectedWatchedDirectoriesRecursive = ts.createMap<number>();
            ...
            ts.projectSystem.checkWatchedDirectoriesDetailed(host, expectedWatchedDirectoriesRecursive, /*recursive*/ true);
            installer.installAll(/*expectedCount*/ 1);
            ts.projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
            host.checkTimeoutQueueLengthAndRun(2);
            ts.projectSystem.checkProjectActualFiles(p, [file1.path, jquery.path, tsconfig.path]);
        ...
        it("Throttle - delayed run install requests", () => {
            ...
            const installer = new (class extends Installer {
                ...
                installWorker(_requestId: number, args: string[], _cwd: string, cb: ts.projectSystem.TI.RequestCompletedAction): void {
                    let typingFiles: (ts.projectSystem.File & {
                        typings: string;
                    })[] = [];
        ...
        it("configured projects discover from node_modules", () => {
            ...
            const host = ts.projectSystem.createServerHost([app, jsconfig, jquery, jqueryPackage, nestedPackage]);
            const installer = new (class extends Installer {
                constructor() {
                    super(host, { globalTypingsCacheLocation: "/tmp", typesRegistry: ts.projectSystem.createTypesRegistry("jquery", "nested") });
                }
        ...
        it("configured projects discover from bower_components", () => {
            ...
            const p = ts.projectSystem.configuredProjectAt(projectService, 0);
            ts.projectSystem.checkProjectActualFiles(p, [app.path, jsconfig.path]);
            const watchedFilesExpected = ts.createMap<number>();
            watchedFilesExpected.set(jsconfig.path, 1); // project files
            watchedFilesExpected.set(ts.projectSystem.libFile.path, 1); // project files
            watchedFilesExpected.set(ts.combinePaths(installer.globalTypingsCacheLocation, "package.json"), 1);
            ts.projectSystem.checkWatchedFilesDetailed(host, watchedFilesExpected);
            ts.projectSystem.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
            ts.projectSystem.checkWatchedDirectoriesDetailed(host, ["/", "/node_modules", "/bower_components"], 1, /*recursive*/ true);
            installer.installAll(/*expectedCount*/ 1);
            ts.projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
            host.checkTimeoutQueueLengthAndRun(2);
            ts.projectSystem.checkProjectActualFiles(p, [app.path, jqueryDTS.path, jsconfig.path]);
        ...
        it("should redo resolution that resolved to '.js' file after typings are installed", () => {
            ...
            const service = ts.projectSystem.createProjectService(host, { typingsInstaller: installer });
            service.openClientFile(file.path);
            ts.projectSystem.checkWatchedFiles(host, [...ts.projectSystem.getConfigFilesToWatch(ts.getDirectoryPath(file.path)), "/a/lib/lib.d.ts"]);
            ...
            ts.projectSystem.checkWatchedDirectories(host, [
                `${ts.tscWatch.projects}/node_modules`,
                `${ts.tscWatch.projects}/a/node_modules`,
                `${ts.tscWatch.projects}/a/b/node_modules`,
                `${ts.tscWatch.projects}/a/node_modules/@types`,
                `${ts.tscWatch.projects}/a/b/node_modules/@types`,
                `${ts.tscWatch.projects}/a/b/bower_components`
            ], /*recursive*/ true);
            service.checkNumberOfProjects({ inferredProjects: 1 });
            ts.projectSystem.checkProjectActualFiles(service.inferredProjects[0], [file.path, commanderJS.path]);
            installer.installAll(/*expectedCount*/ 1);
            for (const name of typeNames) {
                assert.isTrue(host.fileExists(typePath(name)), `typings for '${name}' should be created`);
            }
            host.checkTimeoutQueueLengthAndRun(2);
            ts.projectSystem.checkProjectActualFiles(service.inferredProjects[0], [file.path, ...typeNames.map(typePath)]);
        });
        it("should pick typing names from non-relative unresolved imports", () => {
            ...
            proj.updateGraph();
            assert.deepEqual(proj.cachedUnresolvedImportsPerFile.get((<ts.Path>f1.path)), ["foo", "foo", "foo", "@bar/router", "@bar/common", "@bar/common"]);
            installer.installAll(/*expectedCount*/ 1);
        });
    ...
    describe("unittests:: tsserver:: typingsInstaller:: discover typings", () => {
        ...
        it("should use mappings from safe list", () => {
            ...
            const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path, jquery.path, chroma.path], ts.getDirectoryPath((<ts.Path>app.path)), safeList, ts.emptyMap, { enable: true }, ts.emptyArray, ts.emptyMap);
        ...
        it("should return node for core modules", () => {
            ...
            const cache = ts.createMap<ts.JsTyping.CachedTyping>();
            for (const name of ts.JsTyping.nodeCoreModuleList) {
                const logger = trackingLogger();
                const result = ts.JsTyping.discoverTypings(host, logger.log, [f.path], ts.getDirectoryPath((<ts.Path>f.path)), emptySafeList, cache, { enable: true }, [name, "somename"], ts.emptyMap);
        ...
        it("should use cached locations", () => {
            ...
            const cache = ts.createMapFromTemplate<ts.JsTyping.CachedTyping>({ node: { typingLocation: node.path, version: new ts.Version("1.3.0") } });
            const registry = ts.projectSystem.createTypesRegistry("node");
            const logger = trackingLogger();
            const result = ts.JsTyping.discoverTypings(host, logger.log, [f.path], ts.getDirectoryPath((<ts.Path>f.path)), emptySafeList, cache, { enable: true }, ["fs", "bar"], registry);
            assert.deepEqual(logger.finish(), [
        ...
        it("should gracefully handle packages that have been removed from the types-registry", () => {
            ...
            const cache = ts.createMapFromTemplate<ts.JsTyping.CachedTyping>({ node: { typingLocation: node.path, version: new ts.Version("1.3.0") } });
            const logger = trackingLogger();
            const result = ts.JsTyping.discoverTypings(host, logger.log, [f.path], ts.getDirectoryPath((<ts.Path>f.path)), emptySafeList, cache, { enable: true }, ["fs", "bar"], ts.emptyMap);
            assert.deepEqual(logger.finish(), [
                'Inferred typings from unresolved imports: ["node","bar"]',
        ...
        it("should search only 2 levels deep", () => {
            ...
            const cache = ts.createMap<ts.JsTyping.CachedTyping>();
            const logger = trackingLogger();
            const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path], ts.getDirectoryPath((<ts.Path>app.path)), emptySafeList, cache, { enable: true }, /*unresolvedImports*/ [], ts.emptyMap);
            assert.deepEqual(logger.finish(), [
        ...
        it("should install expired typings", () => {
            ...
            const cache = ts.createMapFromTemplate<ts.JsTyping.CachedTyping>({
            ...
            const logger = trackingLogger();
            const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path], ts.getDirectoryPath((<ts.Path>app.path)), emptySafeList, cache, { enable: true }, ["http", "commander"], registry);
            assert.deepEqual(logger.finish(), [
        ...
        it("should install expired typings with prerelease version of tsserver", () => {
            ...
            const logger = trackingLogger();
            const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path], ts.getDirectoryPath((<ts.Path>app.path)), emptySafeList, cache, { enable: true }, ["http"], registry);
            assert.deepEqual(logger.finish(), [
...
declare module "stream" {
    export type s = string;
            ...
            /*openFiles*/ undefined, ts.arrayIterator([{
                    fileName: file.path,
