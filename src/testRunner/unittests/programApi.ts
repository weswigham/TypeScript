namespace ts {
    function verifyMissingFilePaths(missingPaths: readonly ts.Path[], expected: readonly string[]) {
        ...
        const map = (ts.arrayToSet(expected) as ts.Map<boolean>);
    ...
    describe("unittests:: Program.getMissingFilePaths", () => {
        ...
        const referenceFile = new documents.TextDocument(referenceFileName, "/// <reference path=\"d:/imaginary/nonexistent1.ts\"/>\n" + // Absolute
            "/// <reference path=\"./nonexistent2.ts\"/>\n" + // Relative
            "/// <reference path=\"nonexistent3.ts\"/>\n" + // Unqualified
            "/// <reference path=\"nonexistent4\"/>\n" // No extension
        );
        const testCompilerHost = new fakes.CompilerHost(vfs.createFromFileSystem(Harness.IO, 
        /*ignoreCase*/ true, { documents: [emptyFile, referenceFile], cwd: "d:\\pretend\\" }), { newLine: ts.NewLineKind.LineFeed });
