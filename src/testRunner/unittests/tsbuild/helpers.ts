...
`;
     ...
     */
    export function loadProjectFromDisk(root: string, libContentToAppend?: string): vfs.FileSystem {
     ...
     */
    export function loadProjectFromFiles(files: vfs.FileSet, libContentToAppend?: string): vfs.FileSystem {
        const fs = new vfs.FileSystem(/*ignoreCase*/ true, {
            files,
            cwd: "/",
            meta: { defaultLibLocation: "/lib" },
    ...
    export function getFsWithTime(baseFs: vfs.FileSystem) {
        ...
        host.getWorkspaceRoot = ts.notImplemented;
    ...
    export function generateSourceMapBaselineFiles(fs: vfs.FileSystem, mapFileNames: ts.Iterator<string>) {
        while (true) {
    ...
    function generateBuildInfoSectionBaselineFiles(fs: vfs.FileSystem, buildInfoFileNames: readonly BuildInfoSectionBaselineFiles[]) {
        for (const [file, jsFile, dtsFile] of buildInfoFileNames) {
            if (!fs.existsSync(file))
                continue;
            const buildInfo = ts.getBuildInfo(fs.readFileSync(file, "utf8"));
    ...
    function generateBundleFileSectionInfo(fs: vfs.FileSystem, baselineRecorder: Harness.Compiler.WriterAggregator, bundleFileInfo: ts.BundleFileInfo | undefined, outFile: string | undefined) {
    ...
    export function baselineBuildInfo(configs: readonly ts.ParsedCommandLine[], fs: vfs.FileSystem, writtenFiles: ts.Map<true>) {
        let expectedBuildInfoFiles: BuildInfoSectionBaselineFiles[] | undefined;
        for (const { options } of configs) {
            const out = options.outFile || options.out;
            if (out) {
                const { jsFilePath, declarationFilePath, buildInfoPath } = ts.getOutputPathsForBundle(options, /*forceDts*/ false);
                if (buildInfoPath && writtenFiles.has(buildInfoPath)) {
                    (expectedBuildInfoFiles || (expectedBuildInfoFiles = [])).push([buildInfoPath, jsFilePath, declarationFilePath]);
                }
            }
        }
        if (expectedBuildInfoFiles)
            generateBuildInfoSectionBaselineFiles(fs, expectedBuildInfoFiles);
    }
    export interface TscIncremental {
        buildKind: ts.BuildKind;
        modifyFs: (fs: vfs.FileSystem) => void;
        subScenario?: string;
        commandLineArgs?: readonly string[];
    }
    export interface VerifyTsBuildInput extends ts.TscCompile {
    ...
    export function verifyTscIncrementalEdits({ subScenario, fs, scenario, commandLineArgs, baselineSourceMap, modifyFs, baselineReadFileCalls, incrementalScenarios }: VerifyTsBuildInput) {
        describe(`tsc ${commandLineArgs.join(" ")} ${scenario}:: ${subScenario}`, () => {
            ...
            before(() => {
                ...
                ts.Debug.assert(!!incrementalScenarios.length, `${scenario}/${subScenario}:: No incremental scenarios, you probably want to use verifyTsc instead.`);
            });
            after(() => {
                sys = undefined!;
                tick = undefined!;
            ...
            for (const { buildKind, modifyFs, subScenario: incrementalSubScenario, commandLineArgs: incrementalCommandLineArgs } of incrementalScenarios) {
                describe(incrementalSubScenario || buildKind, () => {
                    let newSys: ts.TscCompileSystem;
                    before(() => {
                        ts.Debug.assert(buildKind !== ts.BuildKind.Initial, "Incremental edit cannot be initial compilation");
                        tick();
                        newSys = ts.tscCompile({
                            scenario,
                    ...
                    it(`Verify emit output file text is same when built clean`, () => {
                        const sys = ts.tscCompile({
                            ...
                            modifyFs: fs => {
                                ...
                                const builder = ts.createSolutionBuilder(host, commandLineArgs, { clean: true });
                                builder.clean();
                            },
                        });
