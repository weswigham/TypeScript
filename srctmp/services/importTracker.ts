...
/* @internal */
export function createImportTracker(sourceFiles: readonly ts.SourceFile[], sourceFilesSet: ts.ReadonlyMap<true>, checker: ts.TypeChecker, cancellationToken: ts.CancellationToken | undefined): ImportTracker {
...
/* @internal */
function getImportersForExport(sourceFiles: readonly ts.SourceFile[], sourceFilesSet: ts.ReadonlyMap<true>, allDirectImports: ts.Map<ImporterOrCallExpression[]>, { exportingModuleSymbol, exportKind }: ExportInfo, checker: ts.TypeChecker, cancellationToken: ts.CancellationToken | undefined): {
            ...
            for (const direct of theseDirectImports) {
                ...
                switch (direct.kind) {
                    ...
                    case ts.SyntaxKind.ImportEqualsDeclaration:
                        handleNamespaceImport(direct, direct.name, ts.hasModifier(direct, ts.ModifierFlags.Export), /*alreadyAddedDirect*/ false);
                        break;
...
/* @internal */
function getDirectImportsMap(sourceFiles: readonly ts.SourceFile[], checker: ts.TypeChecker, cancellationToken: ts.CancellationToken | undefined): ts.Map<ImporterOrCallExpression[]> {
    const map = ts.createMap<ImporterOrCallExpression[]>();
...
export function getImportOrExportSymbol(node: ts.Node, symbol: ts.Symbol, checker: ts.TypeChecker, comingFromExport: boolean): ImportedSymbol | ExportedSymbol | undefined {
    ...
    function getExport(): ExportedSymbol | ImportedSymbol | undefined {
        ...
        function getExportAssignmentExport(ex: ts.ExportAssignment): ExportedSymbol {
    ...
    function getExportKindForDeclaration(node: ts.Node): ExportKind {
        return ts.hasModifier(node, ts.ModifierFlags.Default) ? ExportKind.Default : ExportKind.Named;
    }
}
/* @internal */
function getExportEqualsLocalSymbol(importedSymbol: ts.Symbol, checker: ts.TypeChecker): ts.Symbol {
...
/* @internal */
function isExternalModuleImportEquals(eq: ts.ImportEqualsDeclaration): eq is ts.ImportEqualsDeclaration & {
    moduleReference: {
