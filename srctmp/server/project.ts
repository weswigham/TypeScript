    ...
    for (const info of infos) {
        ...
        switch (info.scriptKind) {
            ...
            case ts.ScriptKind.TS:
                if (ts.fileExtensionIs(info.fileName, ts.Extension.Dts)) {
                    result.dts += 1;
...
export abstract class Project implements ts.LanguageServiceHost, ts.ModuleResolutionHost {
    ...
    private rootFilesMap = ts.createMap<ProjectRootFile>();
    private program: ts.Program | undefined;
    private externalFiles: ts.SortedReadonlyArray<string> | undefined;
    private missingFilesMap: ts.Map<ts.FileWatcher> | undefined;
    private generatedFilesMap: GeneratedFileWatcherMap | undefined;
    private plugins: PluginModuleWithName[] = [];
    /*@internal*/
    private packageJsonFilesMap: ts.Map<ts.FileWatcher> | undefined;
    /*@internal*/
    /**
     ...
     */
    cachedUnresolvedImportsPerFile = ts.createMap<readonly string[]>();
    /*@internal*/
     ...
     */
    private lastReportedFileNames: ts.Map<true> | undefined;
    /**
    ...
    /*@internal*/
    private importSuggestionsCache = ts.Completions.createImportSuggestionsForFileCache();
    /*@internal*/
    ...
    /*@internal*/
    constructor(
    /*@internal*/ readonly projectName: string, readonly projectKind: ProjectKind, readonly projectService: ts.server.ProjectService, private documentRegistry: ts.DocumentRegistry, hasExplicitListOfFiles: boolean, lastFileExceededProgramSize: string | undefined, private compilerOptions: ts.CompilerOptions, public compileOnSaveEnabled: boolean, protected watchOptions: ts.WatchOptions | undefined, directoryStructureHost: ts.DirectoryStructureHost, currentDirectory: string | undefined, customRealpath?: (s: string) => string) {
    ...
    /*@internal*/
    getProbableSymlinks(files: readonly ts.SourceFile[]): ts.ReadonlyMap<string> {
    ...
    resolveModuleNames(moduleNames: string[], containingFile: string, reusedNames?: string[], redirectedReference?: ts.ResolvedProjectReference): (ts.ResolvedModuleFull | undefined)[] {
    ...
    getCompileOnSaveAffectedFileList(scriptInfo: ts.server.ScriptInfo): string[] {
        ...
        return ts.mapDefined(ts.BuilderState.getFilesAffectedBy(this.builderState, (this.program!), scriptInfo.path, this.cancellationToken, data => this.projectService.host.createHash!(data)), // TODO: GH#18217
        // TODO: GH#18217
        // TODO: GH#18217
     ...
     */
    emitFile(scriptInfo: ts.server.ScriptInfo, writeFile: (path: string, data: string, writeByteOrderMark?: boolean) => void): boolean {
    ...
    abstract getTypeAcquisition(): ts.TypeAcquisition;
    ...
    registerFileUpdate(fileName: string) {
        (this.updatedFileNames || (this.updatedFileNames = ts.createMap<true>())).set(fileName, true);
    }
    /*@internal*/
    ...
    updateTypingFiles(typingFiles: ts.SortedReadonlyArray<string>) {
        ts.enumerateInsertsAndDeletes<string, string>(typingFiles, this.typingFiles, ts.getStringComparer(!this.useCaseSensitiveFileNames()), ts.noop, removed => this.detachScriptInfoFromProject(removed));
    ...
    private updateGraphWorker() {
        ...
        const hasNewProgram = this.program && (!oldProgram || (this.program !== oldProgram && !((oldProgram.structureIsReused!) & ts.StructureIsReused.Completely)));
        ...
        if (hasNewProgram) {
            ...
            if (this.generatedFilesMap) {
                ...
                if (isGeneratedFileWatcher(this.generatedFilesMap)) {
                    // --out
                    if (!outPath || !this.isValidGeneratedFileWatcher(ts.removeFileExtension(outPath) + ts.Extension.Dts, this.generatedFilesMap)) {
                        this.clearGeneratedFileWatch();
                    }
                }
                else {
                    ...
                    if (outPath) {
                        ...
                        this.generatedFilesMap.forEach((watcher, source) => {
                            const sourceFile = this.program!.getSourceFileByPath((source as ts.Path));
                            if (!sourceFile ||
                                sourceFile.resolvedPath !== source ||
                                !this.isValidGeneratedFileWatcher(ts.getDeclarationEmitOutputFilePathWorker(sourceFile.fileName, this.compilerOptions, this.currentDirectory, this.program!.getCommonSourceDirectory(), this.getCanonicalFileName), watcher)) {
        ...
        if (!this.importSuggestionsCache.isEmpty()) {
            if (this.hasAddedorRemovedFiles || oldProgram && !oldProgram.structureIsReused) {
                ...
                ts.forEachKey(this.dirtyFilesForSuggestions, fileName => {
    ...
    /*@internal*/
    private sourceFileHasChangedOwnImportSuggestions(oldSourceFile: ts.SourceFile | undefined, newSourceFile: ts.SourceFile | undefined) {
    ...
    /*@internal*/
    private ambientModuleDeclarationsAreEqual(oldSourceFile: ts.SourceFile, newSourceFile: ts.SourceFile) {
    ...
    private createGeneratedFileWatcher(generatedFile: string): GeneratedFileWatcher {
        return {
            ...
            watcher: this.projectService.watchFactory.watchFile(this.projectService.host, generatedFile, () => this.projectService.delayUpdateProjectGraphAndEnsureProjectStructureForOpenFiles(this), ts.PollingInterval.High, this.projectService.getWatchOptions(this), ts.WatchType.MissingGeneratedFile, this)
        };
    }
    ...
    print(counter?: number) {
        ...
        this.writeLog(this.filesToString(this.projectService.logger.hasLevel(ts.server.LogLevel.verbose)));
        this.writeLog("-----------------------------------------------");
    }
    ...
    getChangesSinceVersion(lastKnownVersion?: number): ProjectFilesWithTSDiagnostics {
        ...
        if (this.lastReportedFileNames && lastKnownVersion === this.lastReportedVersion) {
            ...
            const externalFiles = this.getExternalFiles().map(f => ts.server.toNormalizedPath(f));
    ...
    protected enableGlobalPlugins(options: ts.CompilerOptions, pluginConfigOverrides: ts.Map<any> | undefined) {
        const host = this.projectService.host;
        if (!host.require) {
    ...
    protected enablePlugin(pluginConfigEntry: ts.PluginImport, searchPaths: string[], pluginConfigOverrides: ts.Map<any> | undefined) {
        this.projectService.logger.info(`Enabling plugin ${pluginConfigEntry.name} from candidate paths: ${searchPaths.join(",")}`);
        const log = (message: string) => this.projectService.logger.info(message);
        let errorLogs: string[] | undefined;
        const logError = (message: string) => { (errorLogs || (errorLogs = [])).push(message); };
        const resolvedModule = ts.firstDefined(searchPaths, searchPath => <PluginModuleFactory | undefined>Project.resolveModule(pluginConfigEntry.name, searchPath, this.projectService.host, log, logError));
        if (resolvedModule) {
    ...
    /*@internal*/
    getPackageJsonsVisibleToFile(fileName: string, rootDir?: string): readonly ts.PackageJsonInfo[] {
        ...
        const result: ts.PackageJsonInfo[] = [];
        ts.forEachAncestorDirectory(ts.getDirectoryPath(filePath), function processDirectory(directory): boolean | undefined {
    ...
    private watchPackageJsonFile(path: ts.Path) {
        const watchers = this.packageJsonFilesMap || (this.packageJsonFilesMap = ts.createMap());
...
function getUnresolvedImports(program: ts.Program, cachedUnresolvedImportsPerFile: ts.Map<readonly string[]>): ts.SortedReadonlyArray<string> {
    const ambientModules = program.getTypeChecker().getAmbientModules().map(mod => ts.stripQuotes(mod.getName()));
    return ts.sortAndDeduplicate(ts.flatMap(program.getSourceFiles(), sourceFile => extractUnresolvedImportsFromSourceFile(sourceFile, ambientModules, cachedUnresolvedImportsPerFile)));
}
function extractUnresolvedImportsFromSourceFile(file: ts.SourceFile, ambientModules: readonly string[], cachedUnresolvedImportsPerFile: ts.Map<readonly string[]>): readonly string[] {
...
export class InferredProject extends Project {
    private static readonly newName = (() => {
        ...
        return () => {
            const id = nextId;
            nextId++;
            return ts.server.makeInferredProjectName(id);
        };
    })();
    ...
    /*@internal*/
    constructor(projectService: ts.server.ProjectService, documentRegistry: ts.DocumentRegistry, compilerOptions: ts.CompilerOptions, watchOptions: ts.WatchOptions | undefined, projectRootPath: ts.server.NormalizedPath | undefined, currentDirectory: string | undefined, pluginConfigOverrides: ts.Map<any> | undefined) {
...
export class ConfiguredProject extends Project {
    private typeAcquisition!: ts.TypeAcquisition; // TODO: GH#18217
    /* @internal */
    configFileWatcher: ts.FileWatcher | undefined;
    private directoriesWatchedForWildcards: ts.Map<ts.WildcardDirectoryWatcher> | undefined;
    readonly canonicalConfigFilePath: ts.server.NormalizedPath;
    private projectReferenceCallbacks: ts.ResolvedProjectReferenceCallbacks | undefined;
    ...
    /* @internal */
    openFileWatchTriggered = ts.createMap<true>();
    /*@internal*/
    configFileSpecs: ts.ConfigFileSpecs | undefined;
    /*@internal*/
    canConfigFileJsonReportNoInputFiles = false;
    /** Ref count to the project when opened from external project */
    private externalProjectRefCount = 0;
    private projectErrors: ts.Diagnostic[] | undefined;
    ...
    /*@internal*/
    constructor(configFileName: ts.server.NormalizedPath, projectService: ts.server.ProjectService, documentRegistry: ts.DocumentRegistry, cachedDirectoryStructureHost: ts.CachedDirectoryStructureHost) {
        super(configFileName, ProjectKind.Configured, projectService, documentRegistry, 
        /*hasExplicitListOfFiles*/ false, 
        /*lastFileExceededProgramSize*/ undefined, 
        /*compilerOptions*/ {}, 
        /*compileOnSaveEnabled*/ false, 
        /*watchOptions*/ undefined, cachedDirectoryStructureHost, ts.getDirectoryPath(configFileName), projectService.host.realpath && (s => this.getRealpath(s)));
        this.canonicalConfigFilePath = ts.server.asNormalizedPath(projectService.toCanonicalFileName(configFileName));
    }
    /* @internal */
    setResolvedProjectReferenceCallbacks(projectReferenceCallbacks: ts.ResolvedProjectReferenceCallbacks) {
    ...
    private fileExistsIfProjectReferenceDts(file: string) {
        ...
        return source !== undefined ?
            ts.isString(source) ? super.fileExists(source) : true :
            undefined;
    }
    /**
    ...
    private directoryExistsIfProjectReferenceDeclDir(dir: string) {
        ...
        return ts.forEachKey((this.mapOfDeclarationDirectories!), declDirPath => dirPath === declDirPath ||
    ...
    directoryExists(path: string): boolean {
        ...
        if (!this.mapOfDeclarationDirectories) {
            this.mapOfDeclarationDirectories = ts.createMap();
            this.projectReferenceCallbacks.forEachResolvedProjectReference(ref => {
                ...
                if (out) {
                    this.mapOfDeclarationDirectories!.set(ts.getDirectoryPath(this.toPath(out)), true);
                }
                else {
    ...
    private handleDirectoryCouldBeSymlink(directory: string) {
        ...
        const directoryPath = ts.ensureTrailingDirectorySeparator(this.toPath(directory));
        if (this.symlinkedDirectories.has(directoryPath))
            return;
        const real = ts.normalizePath(this.projectService.host.realpath!(directory));
    ...
    private fileOrDirectoryExistsUsingSource(fileOrDirectory: string, isFile: boolean): boolean {
        ...
        return ts.firstDefinedIterator(this.symlinkedDirectories.entries(), ([directoryPath, symlinkedDirectory]) => {
            ...
            if (isFile && result) {
                if (!this.symlinkedFiles)
                    this.symlinkedFiles = ts.createMap();
                // Store the real path for the file'
                const absolutePath = ts.getNormalizedAbsolutePath(fileOrDirectory, this.currentDirectory);
    ...
    createConfigFileWatcher() {
        this.configFileWatcher = this.projectService.watchFactory.watchFile(this.projectService.host, this.getConfigFilePath(), (_fileName, eventKind) => this.projectService.onConfigChangedForConfiguredProject(this, eventKind), ts.PollingInterval.High, this.projectService.getWatchOptions(this), ts.WatchType.ConfigFile, this);
    }
    /**
    ...
    updateGraph(): boolean {
        ...
        switch (reloadLevel) {
            ...
            case ts.ConfigFileProgramReloadLevel.Full:
                this.openFileWatchTriggered.clear();
                const reason = ts.Debug.assertDefined(this.pendingReloadReason);
    ...
    /*@internal*/
    enablePluginsWithOptions(options: ts.CompilerOptions, pluginConfigOverrides: ts.Map<any> | undefined) {
        const host = this.projectService.host;
        if (!host.require) {
            this.projectService.logger.info("Plugins were requested but not running in environment that supports 'require'. Nothing will be loaded");
            return;
    ...
    setTypeAcquisition(newTypeAcquisition: ts.TypeAcquisition): void {
    ...
    /*@internal*/
    watchWildcards(wildcardDirectories: ts.Map<ts.WatchDirectoryFlags>) {
        ...
        (directory, flags) => this.projectService.watchWildcardDirectory((directory as ts.Path), flags, this));
    }
    /*@internal*/
    ...
    hasOpenRef() {
        ...
        return ts.forEachEntry(configFileExistenceInfo.openFilesImpactedByConfigFile, (_value, infoPath) => this.containsScriptInfo((this.projectService.getScriptInfoForPath((infoPath as ts.Path))!))) || false;
    }
    /*@internal*/
    ...
    /*@internal*/
    updateErrorOnNoInputFiles(fileNameResult: ts.ExpandResult) {
...
export class ExternalProject extends Project {
    ...
    private typeAcquisition!: ts.TypeAcquisition; // TODO: GH#18217
    /*@internal*/
    constructor(public externalProjectName: string, projectService: ts.server.ProjectService, documentRegistry: ts.DocumentRegistry, compilerOptions: ts.CompilerOptions, lastFileExceededProgramSize: string | undefined, public compileOnSaveEnabled: boolean, projectFilePath?: string, pluginConfigOverrides?: ts.Map<any>, watchOptions?: ts.WatchOptions) {
        super(externalProjectName, ProjectKind.External, projectService, documentRegistry, 
        /*hasExplicitListOfFiles*/ true, lastFileExceededProgramSize, compilerOptions, compileOnSaveEnabled, watchOptions, projectService.host, ts.getDirectoryPath(projectFilePath || ts.normalizeSlashes(externalProjectName)));
        this.enableGlobalPlugins(this.getCompilerOptions(), pluginConfigOverrides);
    }
    updateGraph() {
