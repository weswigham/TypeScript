...
function createModuleResolutionHost(hasDirectoryExists: boolean, ...files: File[]): ts.ModuleResolutionHost {
    ...
    if (hasDirectoryExists) {
        const directories = ts.createMap<string>();
...
describe("unittests:: moduleResolution:: Node module resolution - relative paths", () => {
    function testLoadAsFile(containingFileName: string, moduleFileNameNoExt: string, moduleName: string): void {
        for (const ext of ts.supportedTSExtensions) {
        ...
        function test(ext: string, hasDirectoryExists: boolean) {
            ...
            for (const e of ts.supportedTSExtensions) {
                if (e === ext) {
                    break;
                }
                else {
    ...
    function testLoadingFromPackageJson(containingFileName: string, packageJsonFileName: string, fieldRef: string, moduleFileName: string, moduleName: string): void {
        ...
        function test(hasDirectoryExists: boolean) {
            ...
            assert.equal(resolution.failedLookupLocations.length, ts.supportedTSExtensions.length);
        }
    }
...
describe("unittests:: moduleResolution:: Files with different casing with forceConsistentCasingInFileNames", () => {
    ...
    function test(files: ts.Map<string>, options: ts.CompilerOptions, currentDirectory: string, useCaseSensitiveFileNames: boolean, rootFiles: string[], expectedDiagnostics: (program: ts.Program) => readonly ts.Diagnostic[]): void {
        const getCanonicalFileName = ts.createGetCanonicalFileName(useCaseSensitiveFileNames);
        if (!useCaseSensitiveFileNames) {
            const oldFiles = files;
            files = ts.createMap<string>();
    ...
    it("should succeed when the same file is referenced using absolute and relative names", () => {
        ...
        test(files, { module: ts.ModuleKind.AMD }, "/a/b", 
        /*useCaseSensitiveFileNames*/ false, ["c.ts", "/a/b/d.ts"], () => ts.emptyArray);
    });
    it("should fail when two files used in program differ only in casing (tripleslash references)", () => {
        ...
        test(files, { module: ts.ModuleKind.AMD, forceConsistentCasingInFileNames: true }, "/a/b", 
        /*useCaseSensitiveFileNames*/ false, ["c.ts", "d.ts"], program => [{
                ...ts.tscWatch.getDiagnosticOfFileFromProgram(program, "c.ts", `/// <reference path="D.ts"/>`.indexOf(`D.ts`), "D.ts".length, ts.Diagnostics.Already_included_file_name_0_differs_from_file_name_1_only_in_casing, "D.ts", "d.ts"),
    ...
    it("should fail when two files used in program differ only in casing (imports)", () => {
        ...
        test(files, { module: ts.ModuleKind.AMD, forceConsistentCasingInFileNames: true }, "/a/b", 
        /*useCaseSensitiveFileNames*/ false, ["c.ts", "d.ts"], program => [{
                ...ts.tscWatch.getDiagnosticOfFileFromProgram(program, "c.ts", `import {x} from "D"`.indexOf(`"D"`), `"D"`.length, ts.Diagnostics.Already_included_file_name_0_differs_from_file_name_1_only_in_casing, "/a/b/D.ts", "d.ts"),
                reportsUnnecessary: undefined
            }]);
    });
    it("should fail when two files used in program differ only in casing (imports, relative module names)", () => {
        ...
        test(files, { module: ts.ModuleKind.CommonJS, forceConsistentCasingInFileNames: true }, "", 
        /*useCaseSensitiveFileNames*/ false, ["moduleA.ts", "moduleB.ts"], program => [{
                ...ts.tscWatch.getDiagnosticOfFileFromProgram(program, "moduleA.ts", `import {x} from "./ModuleB"`.indexOf(`"./ModuleB"`), `"./ModuleB"`.length, ts.Diagnostics.Already_included_file_name_0_differs_from_file_name_1_only_in_casing, "ModuleB.ts", "moduleB.ts"),
                reportsUnnecessary: undefined
            }]);
    });
    ...
    it("should fail when module name in 'require' calls has inconsistent casing", () => {
        ...
        /*useCaseSensitiveFileNames*/ false, ["moduleA.ts", "moduleB.ts", "moduleC.ts"], program => [
            {
                ...ts.tscWatch.getDiagnosticOfFileFromProgram(program, "moduleA.ts", `import a = require("./ModuleC")`.indexOf(`"./ModuleC"`), `"./ModuleC"`.length, ts.Diagnostics.Already_included_file_name_0_differs_from_file_name_1_only_in_casing, "ModuleC.ts", "moduleC.ts"),
                reportsUnnecessary: undefined
            },
            {
                ...ts.tscWatch.getDiagnosticOfFileFromProgram(program, "moduleB.ts", `import a = require("./moduleC")`.indexOf(`"./moduleC"`), `"./moduleC"`.length, ts.Diagnostics.File_name_0_differs_from_already_included_file_name_1_only_in_casing, "moduleC.ts", "ModuleC.ts"),
                reportsUnnecessary: undefined
            }
        ]);
    });
...
import b = require("./moduleB");
        ...
        test(files, { module: ts.ModuleKind.CommonJS, forceConsistentCasingInFileNames: true }, "/a/B/c", 
        /*useCaseSensitiveFileNames*/ false, ["moduleD.ts"], program => [{
                ...ts.tscWatch.getDiagnosticOfFileFromProgram(program, "moduleB.ts", `import a = require("./moduleC")`.indexOf(`"./moduleC"`), `"./moduleC"`.length, ts.Diagnostics.File_name_0_differs_from_already_included_file_name_1_only_in_casing, "/a/B/c/moduleC.ts", "/a/B/c/ModuleC.ts"),
                reportsUnnecessary: undefined
            }]);
    });
...
import b = require("./moduleB");
        ...
        test(files, { module: ts.ModuleKind.CommonJS, forceConsistentCasingInFileNames: true }, "/a/B/c", 
        /*useCaseSensitiveFileNames*/ false, ["moduleD.ts"], () => ts.emptyArray);
    });
    it("should succeed when the two files in program differ only in drive letter in their names", () => {
        ...
        test(files, { module: ts.ModuleKind.CommonJS, forceConsistentCasingInFileNames: true }, "d:/someFolder", 
        /*useCaseSensitiveFileNames*/ false, ["d:/someFolder/moduleA.ts", "d:/someFolder/moduleB.ts"], () => ts.emptyArray);
    });
});
...
describe("unittests:: moduleResolution:: Type reference directive resolution: ", () => {
    ...
    it("Reused program keeps errors", () => {
        ...
        assert.isTrue(program1.structureIsReused === ts.StructureIsReused.Completely);
