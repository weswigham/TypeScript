...
describe("unittests:: tsserver:: occurence highlight on string", () => {
    it("should be marked if only on string values", () => {
        ...
        {
            const highlightRequest = ts.projectSystem.makeSessionRequest<ts.projectSystem.protocol.FileLocationRequestArgs>(ts.projectSystem.CommandNames.Occurrences, { file: file1.path, line: 1, offset: 11 });
        ...
        {
            const highlightRequest = ts.projectSystem.makeSessionRequest<ts.projectSystem.protocol.FileLocationRequestArgs>(ts.projectSystem.CommandNames.Occurrences, { file: file1.path, line: 3, offset: 13 });
            const highlightResponse = (session.executeCommand(highlightRequest).response as ts.projectSystem.protocol.OccurrencesResponseItem[]);
        ...
        {
            const highlightRequest = ts.projectSystem.makeSessionRequest<ts.projectSystem.protocol.FileLocationRequestArgs>(ts.projectSystem.CommandNames.Occurrences, { file: file1.path, line: 4, offset: 14 });
            const highlightResponse = (session.executeCommand(highlightRequest).response as ts.projectSystem.protocol.OccurrencesResponseItem[]);
            assert.isTrue(highlightResponse.length === 2);
            const firstOccurence = highlightResponse[0];
