...
function createSourceFileWithText(fileName: string, sourceText: SourceText, target: ts.ScriptTarget) {
    const file = (<SourceFileWithText>ts.createSourceFile(fileName, sourceText.getFullText(), target));
...
export function newProgram(texts: NamedSourceText[], rootNames: string[], options: ts.CompilerOptions, useGetSourceFileByPath?: boolean): ProgramWithSourceTexts {
    ...
    const program = (<ProgramWithSourceTexts>ts.createProgram(rootNames, options, host));
...
export function updateProgram(oldProgram: ProgramWithSourceTexts, rootNames: readonly string[], options: ts.CompilerOptions, updater: (files: NamedSourceText[]) => void, newTexts?: NamedSourceText[], useGetSourceFileByPath?: boolean) {
    ...
    const program = (<ProgramWithSourceTexts>ts.createProgram(rootNames, options, host, oldProgram));
...
function checkCache<T>(caption: string, program: ts.Program, fileName: string, expectedContent: ts.Map<T> | undefined, getCache: (f: ts.SourceFile) => ts.Map<T> | undefined, entryChecker: (expected: T, original: T) => boolean): void {
...
function checkResolvedModulesCache(program: ts.Program, fileName: string, expectedContent: ts.Map<ts.ResolvedModule | undefined> | undefined): void {
    checkCache("resolved modules", program, fileName, expectedContent, f => f.resolvedModules, ts.checkResolvedModule);
}
function checkResolvedTypeDirectivesCache(program: ts.Program, fileName: string, expectedContent: ts.Map<ts.ResolvedTypeReferenceDirective> | undefined): void {
...
`, "", `var x = 1`)
    ...
    it("successful if change does not affect imports", () => {
        ...
        assert.equal(program1.structureIsReused, ts.StructureIsReused.Completely);
    ...
    it("fails if change affects tripleslash references", () => {
        ...
        assert.equal(program1.structureIsReused, ts.StructureIsReused.SafeModules);
    });
    it("fails if change affects type references", () => {
        const program1 = newProgram(files, ["a.ts"], { types: ["a"] });
        updateProgram(program1, ["a.ts"], { types: ["b"] }, ts.noop);
        assert.equal(program1.structureIsReused, ts.StructureIsReused.Not);
    });
...
/// <reference types="typerefs1" />`;
    ...
    it("succeeds if missing files remain missing", () => {
        ...
        assert.equal(ts.StructureIsReused.Completely, program1.structureIsReused);
    });
    it("fails if missing file is created", () => {
        ...
        assert.equal(ts.StructureIsReused.Not, program1.structureIsReused);
    });
...
describe("unittests:: Reuse program structure:: isProgramUptoDate should return true when there is no change in compiler options and", () => {
    function verifyProgramIsUptoDate(program: ts.Program, newRootFileNames: string[], newOptions: ts.CompilerOptions) {
        const actual = ts.isProgramUptoDate(program, newRootFileNames, newOptions, path => program.getSourceFileByPath(path)!.version, ts.returnFalse, ts.returnFalse, 
    ...
    function verifyProgramWithoutConfigFile(system: ts.System, rootFiles: string[], options: ts.CompilerOptions) {
        const program = ts.createWatchProgram(ts.createWatchCompilerHostOfFilesAndCompilerOptions(rootFiles, options, /*watchOptions*/ undefined, system)).getCurrentProgram().getProgram();
    ...
    function verifyProgramWithConfigFile(system: ts.System, configFileName: string) {
        const program = ts.createWatchProgram(ts.createWatchCompilerHostOfConfigFile(configFileName, {}, /*watchOptionsToExtend*/ undefined, system)).getCurrentProgram().getProgram();
        const { fileNames, options } = (ts.parseConfigFileWithSystem(configFileName, {}, /*watchOptionsToExtend*/ undefined, system, ts.notImplemented)!); // TODO: GH#18217
