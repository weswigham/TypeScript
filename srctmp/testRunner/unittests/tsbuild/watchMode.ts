...
type TsBuildWatchSystem = ts.TestFSWithWatch.TestServerHostTrackingWrittenFiles;
function createTsBuildWatchSystem(fileOrFolderList: readonly ts.TestFSWithWatch.FileOrFolderOrSymLink[], params?: ts.TestFSWithWatch.TestServerHostCreationParameters) {
    return ts.TestFSWithWatch.changeToHostTrackingWrittenFiles(ts.tscWatch.createWatchedSystem(fileOrFolderList, params));
}
export function createSolutionBuilder(system: ts.tscWatch.WatchedSystem, rootNames: readonly string[], defaultOptions?: ts.BuildOptions) {
...
export function createSolutionBuilderWithWatchHost(system: ts.tscWatch.WatchedSystem) {
    const host = ts.createSolutionBuilderWithWatchHost(system);
    host.now = system.now.bind(system);
    return host;
}
function createSolutionBuilderWithWatch(system: TsBuildWatchSystem, rootNames: readonly string[], defaultOptions?: ts.BuildOptions) {
...
describe("unittests:: tsbuild:: watchMode:: program updates", () => {
    ...
    function verifyChangedFiles(actualStamps: OutputFileStamp[], oldTimeStamps: OutputFileStamp[], changedFiles: readonly string[], modifiedTimeStampFiles: readonly string[]) {
        for (let i = 0; i < oldTimeStamps.length; i++) {
            ...
            const expectedIsChanged = ts.contains(changedFiles, actual[0]);
            assert.equal(actual[2], ts.contains(changedFiles, actual[0]), `Expected ${actual[0]} to be written.`);
            if (expectedIsChanged || ts.contains(modifiedTimeStampFiles, actual[0])) {
    ...
    function createSolutionInWatchMode(allFiles: readonly ts.tscWatch.File[], defaultOptions?: ts.BuildOptions, disableConsoleClears?: boolean) {
        ...
        verifyWatches(host);
        ts.tscWatch.checkOutputErrorsInitial(host, ts.emptyArray, disableConsoleClears);
    ...
    function verifyWatches(host: TsBuildWatchSystem) {
        ts.tscWatch.checkWatchedFiles(host, testProjectExpectedWatchedFiles);
        ts.tscWatch.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
        ts.tscWatch.checkWatchedDirectories(host, testProjectExpectedWatchedDirectoriesRecursive, /*recursive*/ true);
    }
    ...
    it("verify building references watches only those projects", () => {
        ...
        ts.tscWatch.checkWatchedFiles(system, testProjectExpectedWatchedFiles.slice(0, testProjectExpectedWatchedFiles.length - tests.length));
        ts.tscWatch.checkWatchedDirectories(system, ts.emptyArray, /*recursive*/ false);
        ts.tscWatch.checkWatchedDirectories(system, testProjectExpectedWatchedDirectoriesRecursive, /*recursive*/ true);
        ts.tscWatch.checkOutputErrorsInitial(system, ts.emptyArray);
    ...
    describe("validates the changes and watched files", () => {
        ...
        function verifyProjectChanges(allFilesGetter: () => readonly ts.tscWatch.File[]) {
            function createSolutionInWatchModeToVerifyChanges(additionalFiles?: readonly [SubProject, string][]) {
                ...
                function verifyChangeAfterTimeout(outputFileStamps: OutputFileStamp[], local?: boolean) {
                    ...
                    verifyChangedFiles(changedCore, outputFileStamps, additionalFiles ?
                    ...
                    [
                        ...
                        ...(additionalFiles ? getOutputFileNames(SubProject.core, "index") : ts.emptyArray)
                    ]);
                    ...
                    verifyChangedFiles(changedLogic, changedCore, additionalFiles || local ? ts.emptyArray :
                        getOutputFileNames(SubProject.logic, "index"), additionalFiles || local ?
                        getOutputFileNames(SubProject.logic, "index") : ts.emptyArray);
                    host.checkTimeoutQueueLengthAndRun(1); // Builds tests
                    const changedTests = getOutputFileStamps(host, additionalFiles);
                    verifyChangedFiles(changedTests, changedLogic, additionalFiles || local ? ts.emptyArray :
                        getOutputFileNames(SubProject.tests, "index"), additionalFiles || local ?
                        getOutputFileNames(SubProject.tests, "index") : ts.emptyArray);
                    host.checkTimeoutQueueLength(0);
                    ts.tscWatch.checkOutputErrorsIncremental(host, ts.emptyArray);
                    verifyWatches();
                }
                function verifyWatches() {
                    ts.tscWatch.checkWatchedFiles(host, additionalFiles ? testProjectExpectedWatchedFiles.concat(newFile.path) : testProjectExpectedWatchedFiles);
                    ts.tscWatch.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
                    ts.tscWatch.checkWatchedDirectories(host, testProjectExpectedWatchedDirectoriesRecursive, /*recursive*/ true);
                }
            }
...
export class someClass2 { }`);
    ...
    it("watches config files that are not present", () => {
        const allFiles = [ts.tscWatch.libFile, ...core, logic[1], ...tests];
        const host = createTsBuildWatchSystem(allFiles, { currentDirectory: projectsLocation });
        createSolutionBuilderWithWatch(host, [`${project}/${SubProject.tests}`]);
        ts.tscWatch.checkWatchedFiles(host, [core[0], core[1], core[2]!, logic[0], ...tests].map(f => f.path.toLowerCase()));
        ts.tscWatch.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
        ts.tscWatch.checkWatchedDirectories(host, [projectPath(SubProject.core)], /*recursive*/ true);
        ts.tscWatch.checkOutputErrorsInitial(host, [
            ts.createCompilerDiagnostic(ts.Diagnostics.File_0_not_found, logic[0].path)
        ]);
        for (const f of [
            ...getOutputFileNames(SubProject.core, "anotherModule"),
        ...
        verifyChangedFiles(changedLogic, initial, getOutputFileNames(SubProject.logic, "index"), ts.emptyArray);
        host.checkTimeoutQueueLengthAndRun(1); // Builds tests
        const changedTests = getOutputFileStamps(host);
        verifyChangedFiles(changedTests, changedLogic, getOutputFileNames(SubProject.tests, "index"), ts.emptyArray);
        host.checkTimeoutQueueLength(0);
        ts.tscWatch.checkOutputErrorsIncremental(host, ts.emptyArray);
        verifyWatches(host);
    });
    it("when referenced using prepend, builds referencing project even for non local change", () => {
        ...
        const coreIndex: ts.tscWatch.File = {
            path: core[1].path,
        ...
        const host = createTsBuildWatchSystem([ts.tscWatch.libFile, ...projectFiles], { currentDirectory: projectsLocation });
...
function myFunc() { return 100; }`, /*isLocal*/ true);
        function verifyChangeInCore(content: string, isLocal?: boolean) {
            ...
            verifyChangedFiles(changedCore, outputFileStamps, getOutputFileNames(SubProject.core, "index"), ts.emptyArray);
            host.checkTimeoutQueueLengthAndRun(1); // Builds logic
            ...
            isLocal ? [changedLogicOutput[0]] : getOutputFileNames(SubProject.logic, "index"), isLocal ? [changedLogicOutput[1]] : ts.emptyArray);
            host.checkTimeoutQueueLength(0);
            ts.tscWatch.checkOutputErrorsIncremental(host, ts.emptyArray);
            verifyWatches();
        }
        ...
        function verifyWatches() {
            ts.tscWatch.checkWatchedFiles(host, projectFiles.map(f => f.path));
            ts.tscWatch.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
            ts.tscWatch.checkWatchedDirectories(host, testProjectExpectedWatchedDirectoriesRecursive, /*recursive*/ true);
        }
    });
...
createSomeObject().message;`
    ...
    describe("reports errors in all projects on incremental compile", () => {
        function verifyIncrementalErrors(defaultBuildOptions?: ts.BuildOptions, disabledConsoleClear?: boolean) {
...
let y: string = 10;`);
            host.checkTimeoutQueueLengthAndRun(1); // Builds logic
            const changedLogic = getOutputFileStamps(host);
            verifyChangedFiles(changedLogic, outputFileStamps, ts.emptyArray, ts.emptyArray);
            host.checkTimeoutQueueLength(0);
            ts.tscWatch.checkOutputErrorsIncremental(host, [
...
let x: string = 10;`);
            ...
            verifyChangedFiles(changedCore, changedLogic, ts.emptyArray, ts.emptyArray);
            host.checkTimeoutQueueLength(0);
            ts.tscWatch.checkOutputErrorsIncremental(host, [
        ...
        private p = 12
            ...
            const outputs = [
                ts.changeExtension(fileWithError.path, ts.Extension.Js),
                ts.changeExtension(fileWithError.path, ts.Extension.Dts),
                ts.changeExtension(fileWithoutError.path, ts.Extension.Js),
                ts.changeExtension(fileWithoutError.path, ts.Extension.Dts),
                `${subProjectLocation}/tsconfig${ts.Extension.TsBuildInfo}`
            ];
            function verifyDtsErrors(host: TsBuildWatchSystem, isIncremental: boolean, expectedErrors: readonly string[]) {
                (isIncremental ? ts.tscWatch.checkOutputErrorsIncremental : ts.tscWatch.checkOutputErrorsInitial)(host, expectedErrors);
            ...
            function createSolutionWithWatch(withFixedError?: true) {
                const files = [ts.tscWatch.libFile, withFixedError ? fileWithFixedError : fileWithError, fileWithoutError, tsconfig];
                ...
                verifyDtsErrors(host, /*isIncremental*/ false, withFixedError ? ts.emptyArray : expectedDtsEmitErrors);
                return host;
            }
            ...
            function fixError(host: TsBuildWatchSystem) {
                ...
                verifyDtsErrors(host, /*isIncremental*/ true, ts.emptyArray);
            }
            ...
            describe("when reporting errors on introducing error", () => {
                function createSolutionWithIncrementalError() {
                    ...
                    host.writtenFiles.clear();
                    incrementalBuild(host);
                    ts.tscWatch.checkOutputErrorsIncremental(host, expectedDtsEmitErrors);
                ...
                function verifyWrittenFile(host: TsBuildWatchSystem, f: string) {
                    assert.isTrue(host.writtenFiles.has(host.toFullPath(f)), `Expected to write ${f}: ${ts.arrayFrom(host.writtenFiles.keys())}`);
                }
                it("when fixing errors only changed file is emitted", () => {
                    const host = createSolutionWithIncrementalError();
                    fixError(host);
                    assert.equal(host.writtenFiles.size, 3, `Expected to write only changed files: ${ts.arrayFrom(host.writtenFiles.keys())}`);
    ...
    describe("tsc-watch and tsserver works with project references", () => {
        describe("invoking when references are already built", () => {
            function verifyWatchesOfProject(host: TsBuildWatchSystem, expectedWatchedFiles: readonly string[], expectedWatchedDirectoriesRecursive: readonly string[], expectedWatchedDirectories?: readonly string[]) {
                ...
                ts.tscWatch.checkWatchedDirectoriesDetailed(host, expectedWatchedDirectories || ts.emptyArray, 1, /*recursive*/ false);
                ts.tscWatch.checkWatchedDirectoriesDetailed(host, expectedWatchedDirectoriesRecursive, 1, /*recursive*/ true);
            }
            function createSolutionOfProject(allFiles: readonly ts.tscWatch.File[], currentDirectory: string, solutionBuilderconfig: string, getOutputFileStamps: (host: TsBuildWatchSystem) => readonly OutputFileStamp[]) {
            ...
            function createSolutionAndWatchModeOfProject(allFiles: readonly ts.tscWatch.File[], currentDirectory: string, solutionBuilderconfig: string, watchConfig: string, getOutputFileStamps: (host: TsBuildWatchSystem) => readonly OutputFileStamp[]) {
                // Build the composite project
                const { host, solutionBuilder } = createSolutionOfProject(allFiles, currentDirectory, solutionBuilderconfig, getOutputFileStamps);
                // Build in watch mode
                const watch = ts.tscWatch.createWatchOfConfigFile(watchConfig, host);
                ts.tscWatch.checkOutputErrorsInitial(host, ts.emptyArray);
                return { host, solutionBuilder, watch };
            }
            function createSolutionAndServiceOfProject(allFiles: readonly ts.tscWatch.File[], currentDirectory: string, solutionBuilderconfig: string, openFileName: string, getOutputFileStamps: (host: TsBuildWatchSystem) => readonly OutputFileStamp[]) {
                // Build the composite project
                const { host, solutionBuilder } = createSolutionOfProject(allFiles, currentDirectory, solutionBuilderconfig, getOutputFileStamps);
                // service
            ...
            function checkProjectActualFiles(service: ts.projectSystem.TestProjectService, configFile: string, expectedFiles: readonly string[]) {
                ts.projectSystem.checkNumberOfProjects(service, { configuredProjects: 1 });
                ts.projectSystem.checkProjectActualFiles((service.configuredProjects.get(configFile.toLowerCase())!), expectedFiles);
            }
            function verifyDependencies(watch: ts.tscWatch.Watch, filePath: string, expected: readonly string[]) {
                ts.tscWatch.checkArray(`${filePath} dependencies`, watch.getBuilderProgram().getAllDependencies(watch().getSourceFile(filePath)!), expected);
            }
            describe("on sample project", () => {
                ...
                const expectedWatchedDirectoriesRecursive = ts.projectSystem.getTypeRootsFromLocation(projectPath(SubProject.tests));
                const expectedProjectFiles = () => [ts.tscWatch.libFile, ...tests, ...logic.slice(1), ...core.slice(1, core.length - 1)].map(f => f.path);
                const expectedProgramFiles = () => [tests[1].path, ts.tscWatch.libFile.path, coreIndexDts, coreAnotherModuleDts, logicIndexDts];
                ...
                function verifyWatches(host: TsBuildWatchSystem, withTsserver?: boolean) {
                    verifyWatchesOfProject(host, withTsserver ?
                        [...core.slice(0, core.length - 1), ...logic, tests[0], ts.tscWatch.libFile].map(f => f.path.toLowerCase()) :
                        [core[0], logic[0], ...tests, ts.tscWatch.libFile].map(f => f.path).concat([coreIndexDts, coreAnotherModuleDts, logicIndexDts].map(f => f.toLowerCase())), expectedWatchedDirectoriesRecursive);
                }
                function verifyScenario(edit: (host: TsBuildWatchSystem, solutionBuilder: ts.SolutionBuilder<ts.EmitAndSemanticDiagnosticsBuilderProgram>) => void, expectedProgramFilesAfterEdit: () => readonly string[], expectedProjectFilesAfterEdit: () => readonly string[]) {
                    it("with tsc-watch", () => {
                        const { host, solutionBuilder, watch } = createSolutionAndWatchMode();
                        edit(host, solutionBuilder);
                        host.checkTimeoutQueueLengthAndRun(1);
                        ts.tscWatch.checkOutputErrorsIncremental(host, ts.emptyArray);
                        ts.tscWatch.checkProgramActualFiles(watch(), expectedProgramFilesAfterEdit());
                    });
                    it("with tsserver", () => {
                ...
                describe("verifies dependencies and watches", () => {
                    it("with tsc-watch", () => {
                        ...
                        verifyDependencies(watch, tests[1].path, expectedProgramFiles().filter(f => f !== ts.tscWatch.libFile.path));
                    });
                    it("with tsserver", () => {
...
function foo() {
                        ...
                        solutionBuilder.invalidateProject((logic[0].path.toLowerCase() as ts.ResolvedConfigFilePath));
...
export function gfoo() {
                ...
                describe("change in project reference config file builds correctly", () => {
                    verifyScenario((host, solutionBuilder) => {
                        ...
                        solutionBuilder.invalidateProject((logic[0].path.toLowerCase() as ts.ResolvedConfigFilePath), ts.ConfigFileProgramReloadLevel.Full);
                        solutionBuilder.buildNextInvalidatedProject();
                    }, () => [tests[1].path, ts.tscWatch.libFile.path, coreIndexDts, coreAnotherModuleDts, projectFilePath(SubProject.logic, "decls/index.d.ts")], expectedProjectFiles);
                });
            });
            describe("on transitive references", () => {
                ...
                function getRootFile(multiFolder: boolean, fileFromDisk: ts.tscWatch.File, multiFolderPath: string): ts.tscWatch.File {
                    return multiFolder ? {
                ...
                function verifyWatchState(host: TsBuildWatchSystem, watch: ts.tscWatch.Watch, expectedProgramFiles: readonly string[], expectedWatchedFiles: readonly string[], expectedWatchedDirectoriesRecursive: readonly string[], dependencies: readonly [string, readonly string[]][], expectedWatchedDirectories?: readonly string[]) {
                    ts.tscWatch.checkProgramActualFiles(watch(), expectedProgramFiles);
                ...
                function getTsConfigFile(multiFolder: boolean, fileFromDisk: ts.tscWatch.File, folder: string): ts.tscWatch.File {
                    if (!multiFolder)
                        return fileFromDisk;
                    return {
                ...
                function verifyTransitiveReferences(multiFolder: boolean) {
                    ...
                    //     writeFile(cTsconfig);
                    // }
                    const allFiles = [ts.tscWatch.libFile, aTs, bTs, cTs, aTsconfig, bTsconfig, cTsconfig, refs];
                    ...
                    const expectedProgramFiles = [cTs.path, ts.tscWatch.libFile.path, aDts, refs.path, bDts];
                    const expectedProjectFiles = [cTs.path, ts.tscWatch.libFile.path, aTs.path, refs.path, bTs.path];
                    ...
                    const expectedWatchedDirectoriesRecursive = [
                        ...
                        ...ts.projectSystem.getTypeRootsFromLocation(multiFolder ? getFilePathInProject(project, "c") : getProjectPath(project))
                    ].map(s => s.toLowerCase());
                    ...
                    function verifyProgram(host: TsBuildWatchSystem, watch: ts.tscWatch.Watch) {
                    ...
                    function verifyProject(host: TsBuildWatchSystem, service: ts.projectSystem.TestProjectService, orphanInfos?: readonly string[]) {
                    ...
                    interface VerifyServerState {
                        ...
                        service: ts.projectSystem.TestProjectService;
                    ...
                    function verifyServerState({ host, service, expectedProjectFiles, expectedProjectWatchedFiles, expectedWatchedDirectoriesRecursive, orphanInfos }: VerifyServerState) {
                        ...
                        const actualOrphan = ts.arrayFrom(ts.mapDefinedIterator(service.filenameToScriptInfo.values(), v => v.containingProjects.length === 0 ? v.fileName : undefined));
                        ...
                        if (orphanInfos && orphanInfos.length) {
                            for (const orphan of orphanInfos) {
                                const info = service.getScriptInfoForPath((orphan as ts.Path));
                    ...
                    interface VerifyScenario {
                        edit: (host: TsBuildWatchSystem, solutionBuilder: ts.SolutionBuilder<ts.EmitAndSemanticDiagnosticsBuilderProgram>) => void;
                    ...
                    function verifyScenario({ edit, expectedEditErrors, expectedProgramFiles, expectedProjectFiles, expectedWatchedFiles, expectedProjectWatchedFiles, expectedWatchedDirectoriesRecursive, dependencies, revert, orphanInfosAfterEdit, orphanInfosAfterRevert }: VerifyScenario) {
                        it("with tsc-watch", () => {
                            ...
                            edit(host, solutionBuilder);
                            host.checkTimeoutQueueLengthAndRun(1);
                            ts.tscWatch.checkOutputErrorsIncremental(host, expectedEditErrors);
                    ...
                    describe("non local edit updates the program and watch correctly", () => {
                        verifyScenario({
                            edit: (host, solutionBuilder) => {
                                ...
                                solutionBuilder.invalidateProject((bTsconfig.path.toLowerCase() as ts.ResolvedConfigFilePath));
                                solutionBuilder.buildNextInvalidatedProject();
                            },
                            expectedEditErrors: ts.emptyArray,
                    ...
                    describe("edit in referenced config file", () => {
                        ...
                        const expectedProgramFiles = [cTs.path, bDts, nrefs.path, refs.path, ts.tscWatch.libFile.path];
                        const expectedProjectFiles = [cTs.path, bTs.path, nrefs.path, refs.path, ts.tscWatch.libFile.path];
                    ...
                    describe("deleting referenced config file", () => {
                        const expectedProgramFiles = [cTs.path, bTs.path, refs.path, ts.tscWatch.libFile.path];
                ...
                describe("when config files are side by side", () => {
                    ...
                    it("when referenced project uses different module resolution", () => {
                        ...
                        const bTsconfig: ts.tscWatch.File = {
                        ...
                        const allFiles = [ts.tscWatch.libFile, aTsFile, bTs, cTsFile, aTsconfigFile, bTsconfig, cTsconfigFile, refs];
                        const aDts = dtsFile("a"), bDts = dtsFile("b");
                        const expectedFiles = [jsFile("a"), aDts, jsFile("b"), bDts, jsFile("c")];
                        const expectedProgramFiles = [cTsFile.path, ts.tscWatch.libFile.path, aDts, refs.path, bDts];
                        ...
                        const expectedWatchedDirectoriesRecursive = [
                            getFilePathInProject(project, "refs"),
                            ...ts.projectSystem.getTypeRootsFromLocation(getProjectPath(project))
                        ].map(s => s.toLowerCase());
                        const defaultDependencies: readonly [string, readonly string[]][] = [
                            [aDts, [aDts]],
                            [bDts, [bDts, aDts]],
    ...
    it("incremental updates in verbose mode", () => {
        ...
        ts.tscWatch.checkOutputErrorsInitial(host, ts.emptyArray, /*disableConsoleClears*/ undefined, [
...
function someFn() { }`);
        ...
        ts.tscWatch.checkOutputErrorsIncremental(host, ts.emptyArray, /*disableConsoleClears*/ undefined, /*logsBeforeWatchDiagnostics*/ undefined, [
...
describe("unittests:: tsbuild:: watchMode:: with demo project", () => {
    ...
    let allFilesExceptBase: ts.tscWatch.File[];
    before(() => {
...
${coreFiles[1].content}`);
        ...
        ].map(ts.tscWatch.hostOutputDiagnostic);
        ts.tscWatch.checkOutputErrors(host, [
            ts.tscWatch.startingCompilationInWatchMode(),
            ts.tscWatch.hostOutputLog(`Projects in this build: \r\n    * core/tsconfig.json\r\n    * animals/tsconfig.json\r\n    * zoo/tsconfig.json\r\n    * tsconfig.json\n\n`),
            ts.tscWatch.hostOutputLog(`Project 'core/tsconfig.json' is out of date because output file 'lib/core/utilities.js' does not exist\n\n`),
            ts.tscWatch.hostOutputLog(`Building project '/user/username/projects/demo/core/tsconfig.json'...\n\n`),
            ...errors,
            ts.tscWatch.hostOutputLog(`Project 'animals/tsconfig.json' can't be built because its dependency 'core' has errors\n\n`),
            ts.tscWatch.hostOutputLog(`Skipping build of project '/user/username/projects/demo/animals/tsconfig.json' because its dependency '/user/username/projects/demo/core' has errors\n\n`),
            ts.tscWatch.hostOutputLog(`Project 'zoo/tsconfig.json' can't be built because its dependency 'animals' was not built\n\n`),
            ts.tscWatch.hostOutputLog(`Skipping build of project '/user/username/projects/demo/zoo/tsconfig.json' because its dependency '/user/username/projects/demo/animals' was not built\n\n`),
            ts.tscWatch.foundErrorsWatching(errors)
        ]);
        verifyWatches(host);
        // Make changes
...
${coreFiles[1].content}`);
        ...
        host.checkTimeoutQueueLength(0);
        ts.tscWatch.checkOutputErrors(host, [
            ts.tscWatch.fileChangeDetected(),
            ts.tscWatch.hostOutputLog(`Project 'core/tsconfig.json' is out of date because output file 'lib/core/utilities.js' does not exist\n\n`),
            ts.tscWatch.hostOutputLog(`Building project '/user/username/projects/demo/core/tsconfig.json'...\n\n`),
            ...newErrors,
            ts.tscWatch.foundErrorsWatching(newErrors)
        ]);
    });
    ...
    function verifyWatches(host: TsBuildWatchSystem) {
        ts.tscWatch.checkWatchedFilesDetailed(host, allFilesExceptBase.map(f => f.path), 1);
        ts.tscWatch.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
        ts.tscWatch.checkWatchedDirectoriesDetailed(host, [`${projectLocation}/core`, `${projectLocation}/animals`, `${projectLocation}/zoo`], 1, 
        /*recursive*/ true);
    }
});
