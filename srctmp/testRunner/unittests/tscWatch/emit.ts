...
function getEmittedLineForMultiFileOutput(file: ts.tscWatch.File, host: ts.tscWatch.WatchedSystem) {
    return `TSFILE: ${file.path.replace(".ts", ".js")}${host.newLine}`;
}
function getEmittedLineForSingleFileOutput(filename: string, host: ts.tscWatch.WatchedSystem) {
    return `TSFILE: ${filename}${host.newLine}`;
}
interface FileOrFolderEmit extends ts.tscWatch.File {
    output?: string;
}
function getFileOrFolderEmit(file: ts.tscWatch.File, getOutput?: (file: ts.tscWatch.File) => string): FileOrFolderEmit {
...
function checkAffectedLines(host: ts.tscWatch.WatchedSystem, affectedFiles: FileOrFolderEmit[], allEmittedFiles: string[]) {
    ...
    ts.tscWatch.checkOutputContains(host, expectedAffectedFiles);
    ts.tscWatch.checkOutputDoesNotContain(host, expectedNonAffectedFiles);
}
describe("unittests:: tsc-watch:: emit with outFile or out setting", () => {
    function createWatchForOut(out?: string, outFile?: string) {
        const host = ts.tscWatch.createWatchedSystem([]);
        ...
        let getOutput: (file: ts.tscWatch.File) => string;
        if (out) {
        ...
        const files = [f1, f2, config, ts.tscWatch.libFile];
        host.reloadFS(files);
        ts.tscWatch.createWatchOfConfigFile(config.path, host);
        const allEmittedLines = getEmittedLines(files);
        ts.tscWatch.checkOutputContains(host, allEmittedLines);
        host.clearOutput();
        f1.content = "let x = 11";
        host.reloadFS(files);
    ...
    function verifyFilesEmittedOnce(useOutFile: boolean) {
        ...
        const host = ts.tscWatch.createWatchedSystem(allfiles);
        const originalWriteFile = host.writeFile.bind(host);
        const mapOfFilesWritten = ts.createMap<number>();
...
describe("unittests:: tsc-watch:: emit for configured projects", () => {
    ...
    interface InitialStateParams {
        ...
        getEmitLine?(file: ts.tscWatch.File, host: ts.tscWatch.WatchedSystem): string;
    ...
    function getInitialState({ configObj = {}, firstCompilationEmitFiles, getEmitLine, getAdditionalFileOrFolder, firstReloadFileList }: InitialStateParams = {}) {
        const host = ts.tscWatch.createWatchedSystem([]);
        const getOutputName = getEmitLine ? (file: ts.tscWatch.File) => getEmitLine(file, host) :
            (file: ts.tscWatch.File) => getEmittedLineForMultiFileOutput(file, host);
        ...
        if (firstCompilationEmitFiles) {
            ...
            ts.tscWatch.checkOutputContains(host, allEmittedFiles);
        }
        host.clearOutput();
        return {
    ...
    it("should return cascaded affected file list", () => {
        const file1Consumer1Consumer1: ts.tscWatch.File = {
    ...
    it("should work fine for files with circular references", () => {
        ...
        const { configFile, getFile, verifyAffectedFiles } = getInitialState({
            ...
            firstReloadFileList: [ts.tscWatch.libFile.path, file1.path, file2.path, configFilePath]
        });
        const file1Emit = getFile(file1.path), file2Emit = getFile(file2.path);
        file1Emit.content += "export var t3 = 10;";
        verifyAffectedFiles([file1Emit, file2Emit], [file1, file2, ts.tscWatch.libFile, configFile]);
    });
    it("should detect removed code file", () => {
        const referenceFile1: ts.tscWatch.File = {
        ...
        const { configFile, getFile, verifyAffectedFiles } = getInitialState({
            ...
            firstReloadFileList: [ts.tscWatch.libFile.path, referenceFile1.path, moduleFile1Path, configFilePath]
        });
        const referenceFile1Emit = getFile(referenceFile1.path);
        verifyAffectedFiles([referenceFile1Emit], [ts.tscWatch.libFile, referenceFile1Emit, configFile]);
    });
    it("should detect non-existing code file", () => {
        ...
        const { configFile, moduleFile2, getFile, verifyAffectedFiles } = getInitialState({
            ...
            getAdditionalFileOrFolder: () => [referenceFile1],
            firstReloadFileList: [ts.tscWatch.libFile.path, referenceFile1.path, configFilePath]
        });
        const referenceFile1Emit = getFile(referenceFile1.path);
        referenceFile1Emit.content += "export var yy = Foo();";
        verifyAffectedFiles([referenceFile1Emit], [ts.tscWatch.libFile, referenceFile1Emit, configFile]);
        // Create module File2 and see both files are saved
        verifyAffectedFiles([referenceFile1Emit, moduleFile2], [ts.tscWatch.libFile, moduleFile2, referenceFile1Emit, configFile]);
    });
});
describe("unittests:: tsc-watch:: emit file content", () => {
    interface EmittedFile extends ts.tscWatch.File {
    ...
    function verifyEmittedFiles(host: ts.tscWatch.WatchedSystem, emittedFiles: EmittedFile[]) {
    ...
    function verifyEmittedFileContents(newLine: string, inputFiles: ts.tscWatch.File[], initialEmittedFileContents: string[], modifyFiles: (files: FileOrFolderEmit[], emitedFiles: EmittedFile[]) => FileOrFolderEmit[], configFile?: ts.tscWatch.File) {
        const host = ts.tscWatch.createWatchedSystem([], { newLine });
        const files = ts.concatenate(ts.map(inputFiles, file => getFileOrFolderEmit(file, fileToConvert => getEmittedLineForMultiFileOutput(fileToConvert, host))), configFile ? [ts.tscWatch.libFile, configFile] : [ts.tscWatch.libFile]);
        ...
        if (configFile) {
            ...
            ts.tscWatch.createWatchOfFilesAndCompilerOptions([files[0].path], host, { listEmittedFiles: true });
        }
        ts.tscWatch.checkOutputContains(host, allEmittedFiles);
    ...
    it("file is deleted and created as part of change", () => {
        ...
        const host = ts.tscWatch.createWatchedSystem(files, { currentDirectory: projectLocation, useCaseSensitiveFileNames: true });
...
describe("unittests:: tsc-watch:: emit with when module emit is specified as node", () => {
    it("when instead of filechanged recursive directory watcher is invoked", () => {
        ...
        const files = [configFile, file1, file2, ts.tscWatch.libFile];
