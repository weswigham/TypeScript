...
function testConvertToAsyncFunction(caption: string, text: string, baselineFolder: string, includeLib?: boolean, expectFailure = false, onlyProvideAction = false) {
    ...
    function runBaseline(extension: ts.Extension) {
        ...
        const context: ts.CodeFixContext = {
            ...
            formatContext: ts.formatting.getFormatContext(ts.testFormatSettings)
        };
        ...
        Harness.Baseline.runBaseline(`${baselineFolder}/${caption}${extension}`, data.join(ts.newLineCharacter));
    }
    function makeLanguageService(f: {
        path: string;
        content: string;
    }, includeLib?: boolean) {
        const host = ts.projectSystem.createServerHost(includeLib ? [f, libFile] : [f]); // libFile is expensive to parse repeatedly - only test when required
