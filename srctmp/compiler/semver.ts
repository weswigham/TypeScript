...
export class Version {
    ...
    constructor(major: number | string, minor = 0, patch = 0, prerelease = "", build = "") {
        if (typeof major === "string") {
            const result = ts.Debug.assertDefined(tryParseComponents(major), "Invalid version");
...
function comparePrerelaseIdentifiers(left: readonly string[], right: readonly string[]) {
    ...
    if (left === right)
        return ts.Comparison.EqualTo;
    if (left.length === 0)
        return right.length === 0 ? ts.Comparison.EqualTo : ts.Comparison.GreaterThan;
    if (right.length === 0)
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (leftIsNumeric || rightIsNumeric) {
            ...
            if (leftIsNumeric !== rightIsNumeric)
                return leftIsNumeric ? ts.Comparison.LessThan : ts.Comparison.GreaterThan;
            // https://semver.org/#spec-item-11
