...
function createResolvedModuleWithFailedLookupLocations(resolved: Resolved | undefined, isExternalLibraryImport: boolean, failedLookupLocations: string[]): ts.ResolvedModuleWithFailedLookupLocations {
    return {
 ...
 */
export function resolveTypeReferenceDirective(typeReferenceDirectiveName: string, containingFile: string | undefined, options: ts.CompilerOptions, host: ts.ModuleResolutionHost, redirectedReference?: ts.ResolvedProjectReference): ts.ResolvedTypeReferenceDirectiveWithFailedLookupLocations {
    ...
    function secondaryLookup(): PathAndPackageId | undefined {
        ...
        if (initialLocationForSecondaryLookup !== undefined) {
            ...
            if (!ts.isExternalModuleNameRelative(typeReferenceDirectiveName)) {
                ...
                const { path: candidate } = ts.normalizePathAndParts(ts.combinePaths(initialLocationForSecondaryLookup, typeReferenceDirectiveName));
...
export interface ModuleResolutionCache extends NonRelativeModuleNameResolutionCache {
    getOrCreateCacheForDirectory(directoryName: string, redirectedReference?: ts.ResolvedProjectReference): ts.Map<ts.ResolvedModuleWithFailedLookupLocations>;
    /*@internal*/ directoryToModuleNameMap: CacheWithRedirects<ts.Map<ts.ResolvedModuleWithFailedLookupLocations>>;
}
/**
...
export interface NonRelativeModuleNameResolutionCache {
    getOrCreateCacheForModuleName(nonRelativeModuleName: string, redirectedReference?: ts.ResolvedProjectReference): PerModuleNameCache;
...
export function createModuleResolutionCache(currentDirectory: string, getCanonicalFileName: (s: string) => string, options?: ts.CompilerOptions): ModuleResolutionCache {
...
export interface CacheWithRedirects<T> {
    ...
    getOrCreateMapOfCacheRedirects(redirectedReference: ts.ResolvedProjectReference | undefined): ts.Map<T>;
    clear(): void;
...
export function createCacheWithRedirects<T>(options?: ts.CompilerOptions): CacheWithRedirects<T> {
    let ownMap: ts.Map<T> = ts.createMap();
    const redirectsMap: ts.Map<ts.Map<T>> = ts.createMap();
    return {
        ownMap,
        redirectsMap,
...
/*@internal*/
export function createModuleResolutionCacheWithMaps(directoryToModuleNameMap: CacheWithRedirects<ts.Map<ts.ResolvedModuleWithFailedLookupLocations>>, moduleNameToDirectoryMap: CacheWithRedirects<PerModuleNameCache>, currentDirectory: string, getCanonicalFileName: ts.GetCanonicalFileName): ModuleResolutionCache {
    ...
    function getOrCreateCacheForDirectory(directoryName: string, redirectedReference?: ts.ResolvedProjectReference) {
        ...
        return getOrCreateCache<ts.Map<ts.ResolvedModuleWithFailedLookupLocations>>(directoryToModuleNameMap, redirectedReference, path, ts.createMap);
    }
    function getOrCreateCacheForModuleName(nonRelativeModuleName: string, redirectedReference?: ts.ResolvedProjectReference): PerModuleNameCache {
    ...
    function createPerModuleNameCache(): PerModuleNameCache {
        const directoryPathMap = ts.createMap<ts.ResolvedModuleWithFailedLookupLocations>();
        return { get, set };
...
export function resolveModuleNameFromCache(moduleName: string, containingFile: string, cache: ModuleResolutionCache): ts.ResolvedModuleWithFailedLookupLocations | undefined {
    const containingDirectory = ts.getDirectoryPath(containingFile);
    const perFolderCache = cache && cache.getOrCreateCacheForDirectory(containingDirectory);
    return perFolderCache && perFolderCache.get(moduleName);
}
export function resolveModuleName(moduleName: string, containingFile: string, compilerOptions: ts.CompilerOptions, host: ts.ModuleResolutionHost, cache?: ModuleResolutionCache, redirectedReference?: ts.ResolvedProjectReference): ts.ResolvedModuleWithFailedLookupLocations {
...
/* @internal */
export function tryResolveJSModule(moduleName: string, initialDir: string, host: ts.ModuleResolutionHost): string | undefined {
...
function getPackageJsonInfo(packageDirectory: string, onlyRecordFailures: boolean, state: ModuleResolutionState): PackageJsonInfo | undefined {
    ...
    const directoryExists = !onlyRecordFailures && ts.directoryProbablyExists(packageDirectory, host);
...
function loadModuleFromNearestNodeModulesDirectory(extensions: Extensions, moduleName: string, directory: string, state: ModuleResolutionState, cache: NonRelativeModuleNameResolutionCache | undefined, redirectedReference: ts.ResolvedProjectReference | undefined): SearchResult<Resolved> {
...
function loadModuleFromNearestNodeModulesDirectoryWorker(extensions: Extensions, moduleName: string, directory: string, state: ModuleResolutionState, typesScopeOnly: boolean, cache: NonRelativeModuleNameResolutionCache | undefined, redirectedReference: ts.ResolvedProjectReference | undefined): SearchResult<Resolved> {
...
function tryLoadModuleUsingPaths(extensions: Extensions, moduleName: string, baseDirectory: string, paths: ts.MapLike<string[]>, loader: ResolutionKindSpecificLoader, onlyRecordFailures: boolean, state: ModuleResolutionState): SearchResult<Resolved> {
    const matchedPattern = ts.matchPatternOrExact(ts.getOwnKeys(paths), moduleName);
    if (matchedPattern) {
 ...
 */
/* @internal */
export function loadModuleFromGlobalCache(moduleName: string, projectName: string | undefined, compilerOptions: ts.CompilerOptions, host: ts.ModuleResolutionHost, globalCache: string): ts.ResolvedModuleWithFailedLookupLocations {
    const traceEnabled = isTraceEnabled(compilerOptions, host);
    if (traceEnabled) {
