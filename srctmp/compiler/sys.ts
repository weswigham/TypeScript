...
/* @internal */
export type HostWatchFile = (fileName: string, callback: FileWatcherCallback, pollingInterval: PollingInterval, options: ts.WatchOptions | undefined) => FileWatcher;
/* @internal */
export type HostWatchDirectory = (fileName: string, callback: DirectoryWatcherCallback, recursive: boolean, options: ts.WatchOptions | undefined) => FileWatcher;
/* @internal */
...
function createUseFsEventsOnParentDirectoryWatchFile(fsWatch: FsWatch, useCaseSensitiveFileNames: boolean): HostWatchFile {
    ...
    const fileWatcherCallbacks = ts.createMultiMap<FileWatcherCallback>();
    const dirWatchers = ts.createMap<DirectoryWatcher>();
    const toCanonicalName = ts.createGetCanonicalFileName(useCaseSensitiveFileNames);
    return nonPollingWatchFile;
    function nonPollingWatchFile(fileName: string, callback: FileWatcherCallback, _pollingInterval: PollingInterval, fallbackOptions: ts.WatchOptions | undefined): FileWatcher {
    ...
    function createDirectoryWatcher(dirName: string, dirPath: string, fallbackOptions: ts.WatchOptions | undefined) {
...
export function createSingleFileWatcherPerName(watchFile: HostWatchFile, useCaseSensitiveFileNames: boolean): HostWatchFile {
    ...
    const cache = ts.createMap<SingleFileWatcher>();
    const callbacksCache = ts.createMultiMap<FileWatcherCallback>();
    const toCanonicalFileName = ts.createGetCanonicalFileName(useCaseSensitiveFileNames);
    return (fileName, callback, pollingInterval, options) => {
        ...
        if (existing) {
            ...
            cache.set(path, {
                watcher: watchFile(fileName, (fileName, eventKind) => ts.forEach(callbacksCache.get(path), cb => cb(fileName, eventKind)), pollingInterval, options),
                refCount: 1
            });
        }
        ...
        return {
            close: () => {
                const watcher = ts.Debug.assertDefined(cache.get(path));
...
/*@internal*/
export let sysLog: (s: string) => void = ts.noop; // eslint-disable-line prefer-const
/*@internal*/
...
export function createDirectoryWatcherSupportingRecursive(host: RecursiveDirectoryWatcherHost): HostWatchDirectory {
    ...
    const cache = ts.createMap<HostDirectoryWatcher>();
    const callbackCache = ts.createMultiMap<{
        dirName: string;
        callback: DirectoryWatcherCallback;
    }>();
    const cacheToUpdateChildWatches = ts.createMap<{
        dirName: string;
        options: ts.WatchOptions | undefined;
    }>();
    let timerToUpdateChildWatches: any;
    const filePathComparer = ts.getStringComparer(!host.useCaseSensitiveFileNames);
    const toCanonicalFilePath = ts.createGetCanonicalFileName(host.useCaseSensitiveFileNames);
     ...
     */
    function createDirectoryWatcher(dirName: string, options: ts.WatchOptions | undefined, callback?: DirectoryWatcherCallback): ChildDirectoryWatcher {
        const dirPath = (toCanonicalFilePath(dirName) as ts.Path);
        ...
        return {
            dirName,
            close: () => {
                const directoryWatcher = ts.Debug.assertDefined(cache.get(dirPath));
                if (callbackToAdd)
    ...
    function invokeCallbacks(dirPath: ts.Path, fileNameOrInvokeMap: string | ts.Map<true>) {
        let fileName: string | undefined;
        let invokeMap: ts.Map<true> | undefined;
    ...
    function nonSyncUpdateChildWatches(dirName: string, dirPath: ts.Path, fileName: string, options: ts.WatchOptions | undefined) {
        // Iterate through existing children and update the watches if needed
    ...
    function scheduleUpdateChildWatches(dirName: string, dirPath: ts.Path, options: ts.WatchOptions | undefined) {
    ...
    function onTimerToUpdateChildWatches() {
        ...
        const invokeMap = ts.createMap<true>();
        while (!timerToUpdateChildWatches && cacheToUpdateChildWatches.size) {
            ...
            invokeCallbacks((dirPath as ts.Path), invokeMap);
            updateChildWatches(dirName, (dirPath as ts.Path), options);
        }
        sysLog(`sysLog:: invokingWatchers:: ${ts.timestamp() - start}ms:: ${cacheToUpdateChildWatches.size}`);
        callbackCache.forEach((callbacks, rootDirName) => {
    ...
    function updateChildWatches(dirName: string, dirPath: ts.Path, options: ts.WatchOptions | undefined) {
        // Iterate through existing children and update the watches if needed
        const parentWatcher = cache.get(dirPath);
        if (parentWatcher) {
     ...
     */
    function watchChildDirectories(parentDir: string, existingChildWatches: ChildWatches, options: ts.WatchOptions | undefined): ChildWatches {
        let newChildWatches: ChildDirectoryWatcher[] | undefined;
        ts.enumerateInsertsAndDeletes<string, ChildDirectoryWatcher>(host.directoryExists(parentDir) ? ts.mapDefined(host.getAccessibleSortedChildDirectories(parentDir), child => {
            const childFullName = ts.getNormalizedAbsolutePath(child, parentDir);
            // Filter our the symbolic link directories since those arent included in recursive watch
            // which is same behaviour when recursive: true is passed to fs.watch
            return !isIgnoredPath(childFullName) && filePathComparer(childFullName, ts.normalizePath(host.realpath(childFullName))) === ts.Comparison.EqualTo ? childFullName : undefined;
        }) : ts.emptyArray, existingChildWatches, (child, childWatcher) => filePathComparer(child, childWatcher.dirName), createAndAddChildDirectoryWatcher, ts.closeFileWatcher, addChildDirectoryWatcher);
    ...
    function isIgnoredPath(path: string) {
        return ts.some(ignoredPaths, searchPath => isInPath(path, searchPath));
    }
...
/*@internal*/
export type FsWatch = (fileOrDirectory: string, entryKind: FileSystemEntryKind, callback: FsWatchCallback, recursive: boolean, fallbackPollingInterval: PollingInterval, fallbackOptions: ts.WatchOptions | undefined) => FileWatcher;
/*@internal*/
...
export function createSystemWatchFunctions({ pollingWatchFile, getModifiedTime, setTimeout, clearTimeout, fsWatch, fileExists, useCaseSensitiveFileNames, fsSupportsRecursiveFsWatch, directoryExists, getAccessibleSortedChildDirectories, realpath, tscWatchFile, useNonPollingWatchers, tscWatchDirectory, }: CreateSystemWatchFunctions): {
    ...
    function watchFile(fileName: string, callback: FileWatcherCallback, pollingInterval: PollingInterval, options: ts.WatchOptions | undefined): FileWatcher {
        options = updateOptionsForWatchFile(options, useNonPollingWatchers);
        const watchFileKind = ts.Debug.assertDefined(options.watchFile);
    ...
    function updateOptionsForWatchFile(options: ts.WatchOptions | undefined, useNonPollingWatchers?: boolean): ts.WatchOptions {
    ...
    function generateWatchFileOptions(watchFile: ts.WatchFileKind, fallbackPolling: ts.PollingWatchKind, options: ts.WatchOptions | undefined): ts.WatchOptions {
    ...
    function watchDirectory(directoryName: string, callback: DirectoryWatcherCallback, recursive: boolean, options: ts.WatchOptions | undefined): FileWatcher {
        if (fsSupportsRecursiveFsWatch) {
            return fsWatch(directoryName, FileSystemEntryKind.Directory, createFsWatchCallbackForDirectoryWatcherCallback(directoryName, callback), recursive, PollingInterval.Medium, ts.getFallbackOptions(options));
        }
    ...
    function nonRecursiveWatchDirectory(directoryName: string, callback: DirectoryWatcherCallback, recursive: boolean, options: ts.WatchOptions | undefined): FileWatcher {
        ts.Debug.assert(!recursive);
        options = updateOptionsForWatchDirectory(options);
        const watchDirectoryKind = ts.Debug.assertDefined(options.watchDirectory);
    ...
    function updateOptionsForWatchDirectory(options: ts.WatchOptions | undefined): ts.WatchOptions {
...
export function patchWriteFileEnsuringDirectory(sys: System) {
    ...
    sys.writeFile = (path, data, writeBom) => ts.writeFileEnsuringDirectories(path, data, !!writeBom, (path, data, writeByteOrderMark) => originalWriteFile.call(sys, path, data, writeByteOrderMark), path => sys.createDirectory(path), path => sys.directoryExists(path));
}
/*@internal*/
...
export interface System {
     ...
     */
    watchFile?(path: string, callback: FileWatcherCallback, pollingInterval?: number, options?: ts.WatchOptions): FileWatcher;
    watchDirectory?(path: string, callback: DirectoryWatcherCallback, recursive?: boolean, options?: ts.WatchOptions): FileWatcher;
    ...
    // For testing
    /*@internal*/ now?(): Date;
    /*@internal*/ require?(baseDir: string, moduleName: string): ts.RequireResult;
}
...
export let sys: System = (() => {
    ...
    function getNodeSystem(): System {
        ...
        const nodeSystem: System = {
            ...
            disableCPUProfiler,
            realpath,
            debugMode: ts.some((<string[]>process.execArgv), arg => /^--(inspect|debug)(-brk)?(=\d+)?$/i.test(arg)),
            ...
            require: (baseDir, moduleName) => {
                try {
                    const modulePath = ts.resolveJSModule(moduleName, baseDir, nodeSystem);
        ...
        function cleanupPaths(profile: import("inspector").Profiler.Profile) {
            ...
            const remappedPaths = ts.createMap<string>();
        ...
        function fsWatch(fileOrDirectory: string, entryKind: FileSystemEntryKind, callback: FsWatchCallback, recursive: boolean, fallbackPollingInterval: PollingInterval, fallbackOptions: ts.WatchOptions | undefined): FileWatcher {
            let options: any;
        ...
        function getAccessibleFileSystemEntries(path: string): ts.FileSystemEntries {
        ...
        function getDirectories(path: string): string[] {
            ts.perfLogger.logEvent("ReadDir: " + path);
            return ts.filter<string>(_fs.readdirSync(path), dir => fileSystemEntryExists(ts.combinePaths(path, dir), FileSystemEntryKind.Directory));
        }
...
if (sys && sys.getEnvironmentVariable) {
    ...
    ts.Debug.currentAssertionLevel = /^development$/i.test(sys.getEnvironmentVariable("NODE_ENV"))
        ? ts.AssertionLevel.Normal
        : ts.AssertionLevel.None;
}
